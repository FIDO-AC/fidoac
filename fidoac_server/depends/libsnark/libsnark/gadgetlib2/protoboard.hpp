/** @file
 *****************************************************************************
 Definition of Protoboard, a "memory manager" for building arithmetic
 constraints
 *****************************************************************************
...
protected:
    ...
    size_t numInputs_;
    ParamsCPtr
        pParams_; // TODO try to refactor this out and use inheritance for
                  // different types of protoboards, for instance
                  // TinyRAMProtoboard : public Protoboard This may not be
                  // trivial because of Gadget multiple inheritance scheme

    Protoboard(const FieldType &fieldType, ParamsCPtr pParams);

public:
    const FieldType fieldType_;
    static ProtoboardPtr create(
        const FieldType &fieldType, ParamsCPtr pParams = NULL)
    {
    ...
    // size_t numVars() const {return
    // constraintSystem_.getUsedVariables().size();} // TODO change to take num
    // from constraintSys_
    ...
    // order to reduce programmer error. We want the programmer to explicitly
    // code what type of constraint she wants.
    void addRank1Constraint(
        const LinearCombination &a,
        const LinearCombination &b,
        const LinearCombination &c,
        const ::std::string &name);
    void addGeneralConstraint(
...
/*************************************************************************************************/
/*
    An abstract class to hold any additional information needed by a specific
   Protoboard. For example a Protoboard specific to TinyRAM will have a class
   ArchParams which will inherit from this class.
*/
class ProtoboardParams
{
public:
