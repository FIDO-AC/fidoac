...
// Forward declaration
void packing_Gadget_R1P_ExhaustiveTest(
    ProtoboardPtr unpackingPB,
    ProtoboardPtr packingPB,
    const int n,
    VariableArray packed,
    VariableArray unpacked,
    GadgetPtr packingGadget,
    GadgetPtr unpackingGadget);
...
TEST(gadgetLib2, R1P_Packing_Gadgets)
{
    ...
    { // test CompressionPacking_Gadget
        ...
        auto packingGadget = CompressionPacking_Gadget::create(
            packingPB, unpacked, packed, PackingMode::PACK);
        auto unpackingGadget = CompressionPacking_Gadget::create(
            unpackingPB, unpacked, packed, PackingMode::UNPACK);
    ...
    { // test IntegerPacking_Gadget
        ...
        VariableArray packed(1, "packed");
        VariableArray unpacked(n, "unpacked");
        auto packingGadget = IntegerPacking_Gadget::create(
            packingPB, unpacked, packed, PackingMode::PACK);
        auto unpackingGadget = IntegerPacking_Gadget::create(
            unpackingPB, unpacked, packed, PackingMode::UNPACK);
        packing_Gadget_R1P_ExhaustiveTest(
            unpackingPB,
            packingPB,
    ...
    GadgetPtr unpackingGadget)
{
    ...
    for (int i = 0; i < 1l << n; ++i) {
        ...
        for (int j = 0; j < n; ++j) {
            bits[j] = i & 1u << j ? 1 : 0;
            packingPB->val(unpacked[j]) =
                bits[j]; // set unpacked bits in the packing protoboard
        }
        unpackingPB->val(packed[0]) =
            i; // set the packed value in the unpacking protoboard
        unpackingGadget->generateWitness();
        packingGadget->generateWitness();
        ASSERT_TRUE(
            unpackingPB->isSatisfied(PrintOptions::DBG_PRINT_IF_NOT_SATISFIED));
        ASSERT_TRUE(packingPB->isSatisfied());
        ASSERT_EQ(
            packingPB->val(packed[0]), i); // check packed value is correct
        for (int j = 0; j < n; ++j) {
            ...
            SCOPED_TRACE(GADGETLIB2_FMT(
                ...
                bits[j]));
            ASSERT_EQ(
                unpackingPB->val(unpacked[j]),
                bits[j]); // check bit correctness
            packingPB->val(unpacked[j]) = unpackingPB->val(unpacked[j]) =
                1 - bits[j]; // flip bit
            ASSERT_FALSE(unpackingPB->isSatisfied());
            ASSERT_FALSE(packingPB->isSatisfied());
            packingPB->val(unpacked[j]) = unpackingPB->val(unpacked[j]) =
                bits[j]; // restore bit
            // special case to test booleanity checks. Cause arithmetic
            // constraints to stay satisfied while ruining Booleanity
            if (j > 0 && bits[j] == 1 && bits[j - 1] == 0) {
                packingPB->val(unpacked[j - 1]) =
                    unpackingPB->val(unpacked[j - 1]) = 2;
                packingPB->val(unpacked[j]) = unpackingPB->val(unpacked[j]) = 0;
                ASSERT_FALSE(unpackingPB->isSatisfied());
                ASSERT_TRUE(packingPB->isSatisfied()); // packing should not
                                                       // enforce Booleanity
                // restore correct state
                packingPB->val(unpacked[j - 1]) =
                    unpackingPB->val(unpacked[j - 1]) = 0;
...
void LogicGadgetExhaustiveTester::runSoundnessCheck()
{
    ...
    SCOPED_TRACE(GADGETLIB2_FMT(
        "Negative (soundness) test failed. curInput: %u, Constraints "
        "are:",
        currentInputValues));
    EXPECT_FALSE(pb->isSatisfied());
}
LogicGadgetExhaustiveTester::LogicGadgetExhaustiveTester(
    ProtoboardPtr pb, size_t numInputs)
    : pb(pb)
    , numInputs(numInputs)
    , inputs(numInputs, "inputs")
    , output("output")
...
void LogicGadgetExhaustiveTester::runExhaustiveTest()
{
    ...
    for (currentInputValues = 0; currentInputValues < (1u << numInputs);
         ++currentInputValues) {
...
AndGadgetExhaustiveTester::AndGadgetExhaustiveTester(
    ProtoboardPtr pb, size_t numInputs)
...
OrGadgetExhaustiveTester::OrGadgetExhaustiveTester(
    ProtoboardPtr pb, size_t numInputs)
    : LogicGadgetExhaustiveTester(pb, numInputs)
{
