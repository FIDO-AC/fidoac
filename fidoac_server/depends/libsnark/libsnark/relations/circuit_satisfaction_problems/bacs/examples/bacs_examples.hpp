...
/**
 * A BACS example comprises a BACS circuit, BACS primary input, and BACS
 * auxiliary input.
 */
template<typename FieldT> struct bacs_example {
 ...
 *
 * This is done by first selecting primary and auxiliary inputs uniformly at
 * random, and then for each gate:
 * - selecting random left and right wires from primary inputs, auxiliary
 * inputs, and outputs of previous gates,
 * - selecting random linear combinations for left and right wires, consisting
 * of 1, 2, 3 or 4 terms each, with random coefficients,
 * - if the gate is an output gate, then adding a random non-output wire to
 * either left or right linear combination, with appropriate coefficient, so
 * that the linear combination evaluates to 0.
 */
template<typename FieldT>
bacs_example<FieldT> generate_bacs_example(
