...
#define MONT_ITERITER(i, j)                                             \
    ...
    "xorq    %[cy], %[cy]            \n\t"                              \
    "addq    %%rdx, %[T1]            \n\t"                              \
    "adcq    %[T0], %[cy]            # cy:T1:tmp[j-1] <-- "             \
    "(X[i] * Y[j] + T1) + M[j] * u + cy * b \n\t"                       \
    "addq    " STR(((j + 1) * 8)) "(%[tmp]), %[T1]       \n\t"          \
    "adcq    $0, %[cy]               # cy:T1:tmp[j-1] <-- "             \
    "(X[i] * Y[j] + T1) + M[j] * u + (tmp[j+1] + cy) * b \n\t"
