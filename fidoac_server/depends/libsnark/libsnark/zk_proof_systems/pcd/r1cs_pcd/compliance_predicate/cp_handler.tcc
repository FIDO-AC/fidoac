...
r1cs_pcd_message_variable<FieldT>::r1cs_pcd_message_variable(
...
void r1cs_pcd_message_variable<FieldT>::update_all_vars()
{
    ...
    for (size_t var_idx = num_vars_at_construction + 1;
         var_idx <= this->pb.num_variables();
         ++var_idx) {
...
template<typename FieldT>
void r1cs_pcd_message_variable<FieldT>::generate_r1cs_witness(
    const std::shared_ptr<r1cs_pcd_message<FieldT>> &message)
{
    all_vars.fill_with_field_elements(
        this->pb, message->as_r1cs_variable_assignment());
}

template<typename FieldT>
r1cs_pcd_local_data_variable<FieldT>::r1cs_pcd_local_data_variable(
    protoboard<FieldT> &pb, const std::string &annotation_prefix)
    : gadget<FieldT>(pb, annotation_prefix)
{
    num_vars_at_construction = pb.num_variables();
...
template<typename FieldT>
void r1cs_pcd_local_data_variable<FieldT>::generate_r1cs_witness(
    const std::shared_ptr<r1cs_pcd_local_data<FieldT>> &local_data)
{
    all_vars.fill_with_field_elements(
        this->pb, local_data->as_r1cs_variable_assignment());
}

template<typename FieldT, typename protoboardT>
compliance_predicate_handler<FieldT, protoboardT>::compliance_predicate_handler(
...
template<typename FieldT, typename protoboardT>
void compliance_predicate_handler<FieldT, protoboardT>::generate_r1cs_witness(
...
template<typename FieldT, typename protoboardT>
r1cs_pcd_compliance_predicate<FieldT> compliance_predicate_handler<
    FieldT,
    protoboardT>::get_compliance_predicate() const
{
    ...
    std::transform(
        incoming_messages.begin(),
        incoming_messages.end(),
        incoming_message_payload_lengths.begin(),
        [](const std::shared_ptr<r1cs_pcd_message_variable<FieldT>> &msg) {
    ...
    const size_t all_but_witness_length =
        ((1 + outgoing_message_payload_length) + 1 +
         (max_arity + std::accumulate(
                          incoming_message_payload_lengths.begin(),
                          incoming_message_payload_lengths.end(),
                          0)) +
         local_data_length);
    ...
    constraint_system.auxiliary_input_size =
        pb.num_variables() - constraint_system.primary_input_size;

    return r1cs_pcd_compliance_predicate<FieldT>(
        name,
        type,
        constraint_system,
...
template<typename FieldT, typename protoboardT>
r1cs_variable_assignment<FieldT> compliance_predicate_handler<
    FieldT,
    protoboardT>::get_full_variable_assignment() const
{
    return pb.full_variable_assignment();
}

template<typename FieldT, typename protoboardT>
std::shared_ptr<r1cs_pcd_message<FieldT>> compliance_predicate_handler<
    FieldT,
    protoboardT>::get_outgoing_message() const
{
...
std::shared_ptr<r1cs_pcd_message<FieldT>> compliance_predicate_handler<
    FieldT,
    protoboardT>::get_incoming_message(const size_t message_idx) const
{
...
template<typename FieldT, typename protoboardT>
std::shared_ptr<r1cs_pcd_local_data<FieldT>> compliance_predicate_handler<
    FieldT,
    protoboardT>::get_local_data() const
{
    return local_data->get_local_data();
}

template<typename FieldT, typename protoboardT>
r1cs_pcd_witness<FieldT> compliance_predicate_handler<FieldT, protoboardT>::
    get_witness() const
{
