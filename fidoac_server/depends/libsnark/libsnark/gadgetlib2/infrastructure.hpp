...
#ifndef _MSC_VER // emulate the MSVC-specific sprintf_s using the standard
                 // snprintf
#define sprintf_s                                                              \
    snprintf // TODO: sprintf_s!=snprintf
             // (http://blog.verg.es/2008/09/sprintfs-is-not-snprintf.html)
#endif
...
{
// someday, if/when MSVC supports C++0x variadic templates, change FMT in
// release version to the following in order to increase efficiency: #define
// GADGETLIB2_FMT(...) ""
...
// declare a function as never returning, to quiet down "control reaches end of
...
/**
 * The ErrorHandling class containimplements the functionality of displaying the
 * content of error messages (including content of call stack when error
 * happened), and exiting the program.
 */
class ErrorHandling
{
public:
...
// Calculates  upper bound of Log2 of a number (number of bits needed to
// represent value)
...
/*constexpr*/ inline int64_t CEIL(double a) { return (int64_t)ceil(a); }
