...
template<typename ppT> class bacs_ppzksnark_proving_key;

template<typename ppT>
std::ostream &operator<<(
    std::ostream &out, const bacs_ppzksnark_proving_key<ppT> &pk);
...
public:
    ...
    bacs_ppzksnark_proving_key(const bacs_ppzksnark_proving_key<ppT> &other) =
        default;
    bacs_ppzksnark_proving_key(bacs_ppzksnark_proving_key<ppT> &&other) =
        default;
    bacs_ppzksnark_proving_key(
    ...
    bacs_ppzksnark_proving_key<ppT> &operator=(
        const bacs_ppzksnark_proving_key<ppT> &other) = default;

    size_t G1_size() const { return r1cs_pk.G1_size(); }

    size_t G2_size() const { return r1cs_pk.G2_size(); }

    size_t G1_sparse_size() const { return r1cs_pk.G1_sparse_size(); }

    size_t G2_sparse_size() const { return r1cs_pk.G2_sparse_size(); }

    size_t size_in_bits() const { return r1cs_pk.size_in_bits(); }

    void print_size() const { r1cs_pk.print_size(); }

    bool operator==(const bacs_ppzksnark_proving_key<ppT> &other) const;
    friend std::ostream &operator<<<ppT>(
        std::ostream &out, const bacs_ppzksnark_proving_key<ppT> &pk);
    friend std::istream &operator>>
        <ppT>(std::istream &in, bacs_ppzksnark_proving_key<ppT> &pk);
};
 ...
 */
template<typename ppT>
using bacs_ppzksnark_processed_verification_key =
    r1cs_ppzksnark_processed_verification_key<ppT>;

/********************************** Key pair *********************************/

/**
 * A key pair for the BACS ppzkSNARK, which consists of a proving key and a
 * verification key.
 */
template<typename ppT> class bacs_ppzksnark_keypair
{
public:
 ...
 */
template<typename ppT> using bacs_ppzksnark_proof = r1cs_ppzksnark_proof<ppT>;
 ...
 *
 * Given a BACS circuit C, this algorithm produces proving and verification keys
 * for C.
 */
template<typename ppT>
 ...
 * Above, C is the BACS circuit that was given as input to the generator
 * algorithm.
 */
template<typename ppT>
...
/*
 ...
 C.num_inputs, whereas weak input consistency requires that |primary_input| <=
 C.num_inputs (and the primary input is implicitly padded with zeros up to
 length C.num_inputs).
 */
 ...
 */
template<typename ppT>
bacs_ppzksnark_processed_verification_key<ppT> bacs_ppzksnark_verifier_process_vk(
    const bacs_ppzksnark_verification_key<ppT> &vk);
 ...
 */
template<typename ppT>
bool bacs_ppzksnark_online_verifier_weak_IC(
 ...
 */
template<typename ppT>
bool bacs_ppzksnark_online_verifier_strong_IC(
    const bacs_ppzksnark_processed_verification_key<ppT> &pvk,
    const bacs_ppzksnark_primary_input<ppT> &primary_input,
    const bacs_ppzksnark_proof<ppT> &proof);
