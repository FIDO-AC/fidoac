 ...
 *****************************************************************************
 ...
 of memory. The third mostly consists of bookkeeping (with some subtleties
 arising from the need to not break zero knowledge).
...
public:
    ...
    r1cs_variable_assignment<FieldT> payload_as_r1cs_variable_assignment()
        const;
    void print() const;

    static size_t unpacked_payload_size_in_bits(
        const ram_architecture_params<ramT> &ap);
};
...
class ram_compliance_predicate_handler
    : public compliance_predicate_handler<
          ram_base_field<ramT>,
          ram_protoboard<ramT>>
{
protected:
...
public:
    ...
    typedef compliance_predicate_handler<
        ram_base_field<ramT>,
        ram_protoboard<ramT>>
        base_handler;
...
private:
    pb_variable<FieldT>
        zero; // TODO: promote linear combinations to first class objects
    ...
    std::shared_ptr<merkle_authentication_path_variable<FieldT, HashT>>
...
public:
    ...
    static std::shared_ptr<r1cs_pcd_message<FieldT>> get_base_case_message(
        const ram_architecture_params<ramT> &ap,
        const ram_boot_trace<ramT> &primary_input);
    static std::shared_ptr<r1cs_pcd_message<FieldT>> get_final_case_msg(
        const ram_architecture_params<ramT> &ap,
        const ram_boot_trace<ramT> &primary_input,
