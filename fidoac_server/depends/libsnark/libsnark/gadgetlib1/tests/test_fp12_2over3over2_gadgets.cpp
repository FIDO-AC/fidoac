...
TEST(Fp12_2over3over2_Test, ConstantOperations)
{
    ...
    using Fp12_variable = Fp12_2over3over2_variable<Fp12T>;
...
TEST(Fp12_2over3over2_Test, SquareGadgetTest)
{
    ...
    protoboard<FieldT> pb;
    Fp12_2over3over2_variable<Fp12T> z_var(pb, "z");
    Fp12_2over3over2_variable<Fp12T> z_squared_var(pb, "z_squared");
    const size_t num_primary_inputs = pb.num_inputs();
    pb.set_input_sizes(num_primary_inputs);

    Fp12_2over3over2_square_gadget<Fp12T> square_gadget(
    ...
    const r1cs_gg_ppzksnark_keypair<pp> keypair =
        r1cs_gg_ppzksnark_generator<pp>(pb.get_constraint_system(), true);
    r1cs_primary_input<libff::Fr<pp>> primary_input = pb.primary_input();
    r1cs_auxiliary_input<libff::Fr<pp>> auxiliary_input = pb.auxiliary_input();
    r1cs_gg_ppzksnark_proof<pp> proof = r1cs_gg_ppzksnark_prover(
        keypair.pk, primary_input, auxiliary_input, true);
    ASSERT_TRUE(r1cs_gg_ppzksnark_verifier_strong_IC<pp>(
...
TEST(Fp12_2over3over2_Test, MulBy024GadgetTest)
{
    ...
    protoboard<FieldT> pb;
    Fp12_2over3over2_variable<Fp12T> z_var(pb, "z");
    Fp2_variable<Fp2T> x0_var(pb, " x0");
    Fp2_variable<Fp2T> x2_var(pb, " x2");
    Fp2_variable<Fp2T> x4_var(pb, " x4");
    Fp12_2over3over2_variable<Fp12T> z_times_x_var(pb, "z_times_x");
    const size_t num_primary_inputs = pb.num_inputs();
    pb.set_input_sizes(num_primary_inputs);

    Fp12_2over3over2_mul_by_024_gadget<Fp12T> mul_024(
...
TEST(Fp12_2over3over2_Test, MulGadgetTest)
{
    ...
    // Multiplication in a circuit
    protoboard<FieldT> pb;
    Fp12_2over3over2_variable<Fp12T> a_var(pb, "a");
    Fp12_2over3over2_variable<Fp12T> b_var(pb, "b");
    Fp12_2over3over2_variable<Fp12T> c_var(pb, "c");
    const size_t num_primary_inputs = pb.num_inputs();
    pb.set_input_sizes(num_primary_inputs);
    Fp12_2over3over2_mul_gadget<Fp12T> a_times_b(
...
TEST(Fp12_2over3over2_Test, InvGadgetTest)
{
    ...
    protoboard<FieldT> pb;
    Fp12_2over3over2_variable<Fp12T> a_var(pb, "a");
    Fp12_2over3over2_variable<Fp12T> a_inv_var(pb, "a.inverse");
    const size_t num_primary_inputs = pb.num_inputs();
    pb.set_input_sizes(num_primary_inputs);
    Fp12_2over3over2_inv_gadget<Fp12T> invert(
...
TEST(Fp12_2over3over2_Test, CyclotomicSquareGadget)
{
    ...
    // Inversion in a circuit
    protoboard<FieldT> pb;
    Fp12_2over3over2_variable<Fp12T> u_var(pb, "u");
    Fp12_2over3over2_variable<Fp12T> u_squared_var(pb, "u_squared");
    const size_t num_primary_inputs = pb.num_inputs();
    pb.set_input_sizes(num_primary_inputs);
    Fp12_2over3over2_cyclotomic_square_gadget<Fp12T> cyclotomic_square_gadget(
