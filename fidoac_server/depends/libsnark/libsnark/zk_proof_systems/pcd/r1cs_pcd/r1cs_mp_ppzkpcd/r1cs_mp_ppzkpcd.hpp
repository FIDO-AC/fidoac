...
template<typename PCD_ppT> class r1cs_mp_ppzkpcd_proving_key;
...
public:
    ...
    std::vector<r1cs_mp_ppzkpcd_compliance_predicate<PCD_ppT>>
    ...
    std::vector<r1cs_ppzksnark_verification_key<B_pp>>
    ...
    r1cs_mp_ppzkpcd_proving_key(
        const r1cs_mp_ppzkpcd_proving_key<PCD_ppT> &other) = default;
    r1cs_mp_ppzkpcd_proving_key(r1cs_mp_ppzkpcd_proving_key<PCD_ppT> &&other) =
        default;
    r1cs_mp_ppzkpcd_proving_key(
    ...
    r1cs_mp_ppzkpcd_proving_key<PCD_ppT> &operator=(
        const r1cs_mp_ppzkpcd_proving_key<PCD_ppT> &other) = default;
...
template<typename PCD_ppT> class r1cs_mp_ppzkpcd_verification_key;
...
public:
    ...
    r1cs_mp_ppzkpcd_verification_key(
        const r1cs_mp_ppzkpcd_verification_key<PCD_ppT> &other) = default;
    r1cs_mp_ppzkpcd_verification_key(
        r1cs_mp_ppzkpcd_verification_key<PCD_ppT> &&other) = default;
    r1cs_mp_ppzkpcd_verification_key(
        const std::vector<r1cs_ppzksnark_verification_key<A_pp>>
    ...
    r1cs_mp_ppzkpcd_verification_key<PCD_ppT> &operator=(
        const r1cs_mp_ppzkpcd_verification_key<PCD_ppT> &other) = default;
...
template<typename PCD_ppT> class r1cs_mp_ppzkpcd_processed_verification_key;
...
public:
    ...
    std::vector<r1cs_ppzksnark_processed_verification_key<A_pp>>
    ...
    r1cs_mp_ppzkpcd_processed_verification_key(
        const r1cs_mp_ppzkpcd_processed_verification_key<PCD_ppT> &other) =
        default;
    r1cs_mp_ppzkpcd_processed_verification_key(
    ...
    r1cs_mp_ppzkpcd_processed_verification_key<PCD_ppT> &operator=(
        const r1cs_mp_ppzkpcd_processed_verification_key<PCD_ppT> &other) =
        default;
...
/**
 * A key pair for the R1CS (multi-predicate) ppzkPC, which consists of a proving
 * key and a verification key.
 */
template<typename PCD_ppT> class r1cs_mp_ppzkpcd_keypair
{
public:
...
template<typename ppT> class r1cs_mp_ppzkpcd_proof;

template<typename ppT>
std::ostream &operator<<(
    std::ostream &out, const r1cs_mp_ppzkpcd_proof<ppT> &proof);
 ...
 */
template<typename PCD_ppT> class r1cs_mp_ppzkpcd_proof
{
public:
    ...
    r1cs_mp_ppzkpcd_proof(
        const size_t compliance_predicate_idx,
    ...
    friend std::istream &operator>>
        <PCD_ppT>(std::istream &in, r1cs_mp_ppzkpcd_proof<PCD_ppT> &proof);
};
 ...
 *
 * Given a vector of compliance predicates, this algorithm produces proving and
 * verification keys for the vector.
 */
template<typename PCD_ppT>
...
/*
  Below are two variants of verifier algorithm for the R1CS (multi-predicate)
  ppzkPCD.
