 ...
 *****************************************************************************
 ...
 These interfaces have been designed to allow later adding other fields or
 constraint structures while allowing high level design to stay put.

 A gadget represents (and generates) the constraints, constraint "wiring", and
 witness for a logical task. This is best explained using the physical design of
 a printed circuit. The Protoboard is the board on which we will "solder" our
 circuit. The wires (implemented by Variables) can hold any element of the
 underlying field. Each constraint enforces a relation between wires. These can
 be thought of as gates.

 The delegation of tasks is as follows:

 -   Constructor - Allocates all Variables to a Protoboard. Creates all
 sub-gadgets that will be needed and wires their inputs and outputs.
 ...
 -   generateWitness - Generates an assignment for all non-input Variables which
 is consistent with the assignment of the input Variables and satisfies all of
 the constraints. In essence, this computes the logical function of the Gadget.

 -   create - A static factory method used for construction of the Gadget. This
 is used in order to create a Gadget without explicit knowledge of the
...
/**
 Gadget class, representing the constraints and witness generation for a logical
 task.

 Gadget hierarchy:
 (Here and elsewhere: R1P = Rank 1 constraints over a prime-characteristic
 field.) Gadgets have a somewhat cumbersome class hierarchy, for the sake of
 clean gadget construction. (1) A field agnostic, concrete (as opposed to
 interface) gadget will derive from Gadget. For instance NAND needs only AND and
 NOT and does not care about the field, thus it derives from Gadget. (2) Field
 specific interface class R1P_Gadget derives from Gadget using virtual
     inheritance, in order to avoid the Dreaded Diamond problem (see
     http://stackoverflow.com/a/21607/1756254 for more info)
 (3) Functional interface classes such as LooseMUX_GadgetBase virtually derive
 from Gadget and define special gadget functionality. For gadgets with no
 special interfaces we use the macro CREATE_GADGET_BASE_CLASS() for the sake of
 code consistency (these gadgets can work the same without this base class).
 This is an interface only and the implementation of AND_Gadget is field
 specific. (4) These field specific gadgets will have a factory class with
 static method create, such as AND_Gadget::create(...) in order to agnostically
 create this gadget for use by a field agnostic gadget. (5) Concrete field
 dependent gadgets derive via multiple inheritance from two interfaces. e.g.
 R1P_AND_Gadget derives from both AND_Gadget and R1P_Gadget. This was done to
 allow usage of AND_Gadget's field agnostic create() method and R1P_Gadget's
 field specific val() method.
*/
class Gadget
{
private:
...
public:
    ...
    virtual void init() = 0;
    /* generate constraints must have this interface, however generateWitness
       for some gadgets (like CTime) will take auxiliary information (like
       memory contents). We do not want to force the interface for
       generateWitness but do want to make sure it is never invoked from base
       class.
    */
    virtual void generateConstraints() = 0;
    virtual void generateWitness(); // Not abstract as this method may have
                                    // different signatures.
    void addUnaryConstraint(
        const LinearCombination &a, const ::std::string &name);
    void addRank1Constraint(
...
typedef ::std::shared_ptr<Gadget>
    GadgetPtr; // Not a unique_ptr because sometimes we need to cast
...
/*
   We use multiple inheritance in order to use much needed syntactic sugar. We
   want val() to be able to return different types depending on the field so we
   need to differentiate the interfaces between R1P and other fields. We also
   want the interfaces of specific logical gadgets (for instance AND_Gadget
   which has n inputs and 1 output) in order to construct higher level gadgets
   without specific knowledge of the underlying field. Both interfaces (for
   instance R1P_gadget and AND_Gadget) inherit from Gadget using virtual
   inheritance (this means only one instance of Gadget will be created. For a
   more thorough discussion on virtual inheritance see
...
public:
    ...
    virtual void addRank1Constraint(
        const LinearCombination &a,
        const LinearCombination &b,
        const LinearCombination &c,
        const ::std::string &name);
...
private:
    virtual void init() = 0; // private in order to force programmer to invoke
                             // from a Gadget* only
...
CREATE_GADGET_FACTORY_CLASS_3(
    InnerProduct_Gadget, VariableArray, A, VariableArray, B, Variable, result);
...
/*
    ...
    index is in bounds, we can also set success_flag to 0 -> result will be
   forced to 0
*/
...
// In R1P compression and arithmetic packing are implemented the same, hence
// this gadget simply instantiates an R1P_CompressionPacking_Gadget
...
CREATE_GADGET_FACTORY_CLASS_3(
    EqualsConst_Gadget, FElem, n, LinearCombination, input, Variable, result);
...
CREATE_GADGET_FACTORY_CLASS_5(
    Comparison_Gadget, // TODO uncomment this
    size_t,
    wordBitSize,
    PackedWord,
    lhs,
