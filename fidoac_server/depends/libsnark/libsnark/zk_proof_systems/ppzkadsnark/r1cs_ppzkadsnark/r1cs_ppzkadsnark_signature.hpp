...
template<typename ppT> class kpT
{
public:
...
template<typename ppT> kpT<ppT> sigGen(void);

template<typename ppT>
r1cs_ppzkadsnark_sigT<ppT> sigSign(
    const r1cs_ppzkadsnark_skT<ppT> &sk,
    const labelT &label,
    const libff::G2<snark_pp<ppT>> &Lambda);

template<typename ppT>
bool sigVerif(
    const r1cs_ppzkadsnark_vkT<ppT> &vk,
    const labelT &label,
    const libff::G2<snark_pp<ppT>> &Lambda,
