 ...
 *****************************************************************************
 ...
 Implementation of interfaces for a gadget that can be created from an R1CS
 constraint system.
    ...
    for (auto va : vars) {
        ...
        for (auto v : va) {
            ...
            if (v.index != 0) {
                ...
                const std::map<size_t, std::string>::const_iterator it =
                    cs.variable_annotations.find(cs_var_idx);

                std::string annotation =
                    FMT(annotation_prefix, " variable_%zu", cs_var_idx);
...
void gadget_from_r1cs<FieldT>::generate_r1cs_constraints()
{
    for (size_t i = 0; i < cs.num_constraints(); ++i) {
        ...
        for (const linear_term<FieldT> &t : constr.a.terms) {
            translated_constr.a.terms.emplace_back(linear_term<FieldT>(
                pb_variable<FieldT>(cs_to_vars[t.index]), t.coeff));
        }

        for (const linear_term<FieldT> &t : constr.b.terms) {
            translated_constr.b.terms.emplace_back(linear_term<FieldT>(
                pb_variable<FieldT>(cs_to_vars[t.index]), t.coeff));
        }

        for (const linear_term<FieldT> &t : constr.c.terms) {
            translated_constr.c.terms.emplace_back(linear_term<FieldT>(
                pb_variable<FieldT>(cs_to_vars[t.index]), t.coeff));
        }

        std::string annotation =
            FMT(this->annotation_prefix, " constraint_%zu", i);
...
template<typename FieldT>
void gadget_from_r1cs<FieldT>::generate_r1cs_witness(
    ...
    const r1cs_auxiliary_input<FieldT> &auxiliary_input)
{
    ...
    for (size_t i = 0; i < auxiliary_input.size(); ++i) {
        this->pb.val(pb_variable<FieldT>(
            cs_to_vars[primary_input.size() + i + 1])) = auxiliary_input[i];
    }
}
