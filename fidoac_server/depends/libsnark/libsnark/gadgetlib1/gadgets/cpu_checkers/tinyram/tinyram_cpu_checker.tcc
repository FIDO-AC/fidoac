...
tinyram_cpu_checker<FieldT>::tinyram_cpu_checker(
    ...
    , next_has_accepted(next_has_accepted)
{
    ...
    arg2idx = pb_variable_array<FieldT>(
        pc_val_it, pc_val_it + pb.ap.reg_arg_or_imm_width());
    std::advance(pc_val_it, pb.ap.reg_arg_or_imm_width());
    std::advance(pc_val_it, pb.ap.instruction_padding_width());
    arg1idx =
        pb_variable_array<FieldT>(pc_val_it, pc_val_it + pb.ap.reg_arg_width());
    ...
    opcode =
        pb_variable_array<FieldT>(pc_val_it, pc_val_it + pb.ap.opcode_width());
    ...
    for (size_t i = 0; i < pb.ap.k; ++i) {
        prev_registers.emplace_back(word_variable_gadget<FieldT>(
            pb,
            pb_variable_array<FieldT>(
                prev_state.begin() + i * pb.ap.w,
                prev_state.begin() + (i + 1) * pb.ap.w),
            FMT(annotation_prefix, " prev_registers_%zu", i)));
        next_registers.emplace_back(word_variable_gadget<FieldT>(
            pb,
            pb_variable_array<FieldT>(
                next_state.begin() + i * pb.ap.w,
                next_state.begin() + (i + 1) * pb.ap.w),
            FMT(annotation_prefix, " next_registers_%zu", i)));
    ...
    /* decode arguments */
    prev_pc_addr_as_word_variable.reset(new word_variable_gadget<FieldT>(
        pb,
        prev_pc_addr,
        FMT(annotation_prefix, " prev_pc_addr_as_word_variable")));
    desval.reset(new word_variable_gadget<FieldT>(
        pb, FMT(annotation_prefix, " desval")));
    arg1val.reset(new word_variable_gadget<FieldT>(
        pb, FMT(annotation_prefix, " arg1val")));
    arg2val.reset(new word_variable_gadget<FieldT>(
        pb, FMT(annotation_prefix, " arg2val")));

    decode_arguments.reset(new argument_decoder_gadget<FieldT>(
        pb,
        arg2_is_imm,
        desidx,
        arg1idx,
    ...
    opcode_indicators.allocate(
        pb,
        1ul << pb.ap.opcode_width(),
        FMT(annotation_prefix, " opcode_indicators"));

    /* perform the ALU operations */
    instruction_results.allocate(
        pb,
        1ul << pb.ap.opcode_width(),
        FMT(annotation_prefix, " instruction_results"));
    instruction_flags.allocate(
        pb,
        1ul << pb.ap.opcode_width(),
        FMT(annotation_prefix, " instruction_flags"));

    ALU.reset(new ALU_gadget<FieldT>(
        pb,
        opcode_indicators,
    ...
    ls_prev_val_as_doubleword_variable.reset(
        new doubleword_variable_gadget<FieldT>(
            pb,
            ls_prev_val,
            FMT(annotation_prefix, " ls_prev_val_as_doubleword_variable")));
    ls_next_val_as_doubleword_variable.reset(
        new doubleword_variable_gadget<FieldT>(
            pb,
            ls_next_val,
            FMT(annotation_prefix, " ls_next_val_as_doubleword_variable")));
    memory_subaddress.reset(new dual_variable_gadget<FieldT>(
        pb,
        pb_variable_array<FieldT>(
            arg2val->bits.begin(), arg2val->bits.begin() + pb.ap.subaddr_len()),
        FMT(annotation_prefix, " memory_subaddress")));

    memory_subcontents.allocate(
        pb, FMT(annotation_prefix, " memory_subcontents"));
    ...
    check_memory.reset(new memory_masking_gadget<FieldT>(
        pb,
    ...
    next_pc_addr_as_word_variable.reset(new word_variable_gadget<FieldT>(
        pb,
        next_pc_addr,
        FMT(annotation_prefix, " next_pc_addr_as_word_variable")));

    consistency_enforcer.reset(new consistency_enforcer_gadget<FieldT>(
        pb,
        opcode_indicators,
        instruction_results,
        instruction_flags,
...
void tinyram_cpu_checker<FieldT>::generate_r1cs_constraints()
{
    ...
    for (size_t i = 0; i < 1ul << this->pb.ap.opcode_width(); ++i) {
        this->pb.add_r1cs_constraint(
            r1cs_constraint<FieldT>(
                opcode_indicators[i], pb_packing_sum<FieldT>(opcode) - i, 0),
    ...
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(
            next_has_accepted, 1 - opcode_indicators[tinyram_opcode_ANSWER], 0),
    ...
    */
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(
            prev_tape1_exhausted, 1 - next_tape1_exhausted, 0),
        FMT(this->annotation_prefix,
    ...
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(
            read_not1, 1 - instruction_flags[tinyram_opcode_READ], 0),
...
void tinyram_cpu_checker<FieldT>::generate_r1cs_witness_address()
{
    ...
    /* clear out ls_addr and fill with everything of arg2val except the
     * subaddress */
    ls_addr.fill_with_bits_of_field_element(
        this->pb,
        this->pb.val(arg2val->packed).as_ulong() >> this->pb.ap.subaddr_len());
}

template<typename FieldT>
void tinyram_cpu_checker<FieldT>::generate_r1cs_witness_other(
    ...
    const tinyram_input_tape_iterator &aux_end)
{
    ...
    const size_t opcode_val =
        opcode.get_field_element_from_bits(this->pb).as_ulong();
    for (size_t i = 0; i < 1ul << this->pb.ap.opcode_width(); ++i) {
        this->pb.val(opcode_indicators[i]) =
            (i == opcode_val ? FieldT::one() : FieldT::zero());
    }
    ...
    const size_t prev_doubleword =
        this->pb.val(ls_prev_val_as_doubleword_variable->packed).as_ulong();
    const size_t subaddress =
        this->pb.val(memory_subaddress->packed).as_ulong();

    if (this->pb.val(opcode_indicators[tinyram_opcode_LOADB]) ==
        FieldT::one()) {
        ...
        this->pb.val(opcode_indicators[tinyram_opcode_STOREB]) ==
        FieldT::one()) {
        const size_t stored_byte =
            (this->pb.val(desval->packed).as_ulong()) & 0xFF;
        this->pb.val(memory_subcontents) = FieldT(stored_byte);
    } else if (
        this->pb.val(opcode_indicators[tinyram_opcode_STOREW]) ==
        FieldT::one()) {
        ...
        const bool access_is_word0 =
            (this->pb.val(*memory_subaddress->bits.rbegin()) == FieldT::zero());
        const size_t loaded_word =
            (prev_doubleword >> (access_is_word0 ? 0 : this->pb.ap.w)) &
            ((1ul << this->pb.ap.w) - 1);
        this->pb.val(instruction_results[tinyram_opcode_LOADW]) = FieldT(
    ...
    this->pb.val(read_not1) =
        this->pb.val(opcode_indicators[tinyram_opcode_READ]) *
        (FieldT::one() - this->pb.val(arg2val->packed));
    ...
    /* finally set has_accepted to 1 if both the opcode is ANSWER and arg2val is
     * 0 */
    this->pb.val(next_has_accepted) =
        (this->pb.val(opcode_indicators[tinyram_opcode_ANSWER]) ==
             FieldT::one() &&
         this->pb.val(arg2val->packed) == FieldT::zero())
            ? FieldT::one()
            : FieldT::zero();
}

template<typename FieldT> void tinyram_cpu_checker<FieldT>::dump() const
{
    printf(
        "   pc = %lu, flag = %lu\n",
    ...
    for (size_t j = 0; j < this->pb.ap.k; ++j) {
        printf(
            "r%zu = %2lu ",
            j,
            this->pb.val(prev_registers[j].packed).as_ulong());
    }
    printf("\n");
