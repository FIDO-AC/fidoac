...
template<typename FieldT> void dump_constraints(const protoboard<FieldT> &pb)
{
#ifdef DEBUG
    ...
    const std::string &annotation_A, const std::string &annotation_B)
{
    ...
    r1cs_example<FieldT_A> example =
        generate_r1cs_example_with_field_input<FieldT_A>(
    ...
    const r1cs_ppzksnark_keypair<ppT_A> keypair =
        r1cs_ppzksnark_generator<ppT_A>(example.constraint_system);
    const r1cs_ppzksnark_proof<ppT_A> pi = r1cs_ppzksnark_prover<ppT_A>(
        keypair.pk, example.primary_input, example.auxiliary_input);
    bool bit = r1cs_ppzksnark_verifier_strong_IC<ppT_A>(
    ...
    r1cs_ppzksnark_verifier_gadget<ppT_B> verifier(
    ...
    const std::string &annotation_A, const std::string &annotation_B)
{
    ...
    protoboard<FieldT_B> pb;
    r1cs_ppzksnark_preprocessed_r1cs_ppzksnark_verification_key_variable<ppT_B>
    ...
    r1cs_ppzksnark_online_verifier_gadget<ppT_B> online_verifier(
        pb,
        hardcoded_vk,
        primary_input_bits,
        elt_size,
...
void test_mul(const std::string &annotation)
{
    ...
    for (size_t i = 0; i < 10; ++i) {
        ...
        ASSERT_EQ(res, x_val * y_val);
        ASSERT_TRUE(pb.is_satisfied());
    }
    printf(
        "number of constraints for %s_mul = %zu\n",
        annotation.c_str(),
        pb.num_constraints());
}
...
void test_sqr(const std::string &annotation)
{
    ...
    for (size_t i = 0; i < 10; ++i) {
        ...
        ASSERT_EQ(res, x_val.squared());
        ASSERT_TRUE(pb.is_satisfied());
    }
    printf(
        "number of constraints for %s_sqr = %zu\n",
        annotation.c_str(),
        pb.num_constraints());
}
...
void test_cyclotomic_sqr(const std::string &annotation)
{
    ...
    printf(
        "number of constraints for %s_cyclotomic_sqr = %zu\n",
        annotation.c_str(),
        pb.num_constraints());
}
...
void test_Frobenius(const std::string &annotation)
{
    ...
    for (size_t i = 0; i < 100; ++i) {
        ...
        ASSERT_EQ(res, x_val.Frobenius_map(i));
        ASSERT_TRUE(pb.is_satisfied());
    }
...
template<typename ppT> void test_full_pairing(const std::string &annotation)
{
    ...
    mnt_miller_loop_gadget<ppT> miller(
        pb, prec_P, prec_Q, miller_result, "miller");
    ...
    PROFILE_CONSTRAINTS(pb, "final exp") { finexp.generate_r1cs_constraints(); }
    PRINT_CONSTRAINT_PROFILING();
    ...
    libff::Fqk<other_curve<ppT>> native_miller_result =
        other_curve<ppT>::affine_ate_miller_loop(native_prec_P, native_prec_Q);

    libff::Fqk<other_curve<ppT>> native_finexp_result =
        other_curve<ppT>::final_exponentiation(native_miller_result);
    printf("Must match:\n");
    finexp.result->get_element().print();
    native_finexp_result.print();

    ASSERT_EQ(finexp.result->get_element(), native_finexp_result);

    printf(
        "number of constraints for full pairing (Fr is %s)  = %zu\n",
        annotation.c_str(),
        pb.num_constraints());
}
...
void test_full_precomputed_pairing(const std::string &annotation)
{
    ...
    printf(
        "number of constraints for full precomputed pairing (Fr is %s)  = "
        "%zu\n",
        annotation.c_str(),
        pb.num_constraints());
}

TEST(R1CSPPZKSNARK, TestVerifierGadget)
{
    ...
    test_cyclotomic_sqr<
        libff::mnt4_pp,
        Fp4_variable,
        Fp4_cyclotomic_sqr_gadget>("mnt4_Fp4");
    test_exponentiation_gadget<
        ...
        Fp4_sqr_gadget,
        libff::mnt4_q_limbs>(
        libff::mnt4_final_exponent_last_chunk_abs_of_w0, "mnt4_Fq4");
