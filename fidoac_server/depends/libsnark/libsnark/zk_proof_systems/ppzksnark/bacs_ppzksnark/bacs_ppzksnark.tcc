...
bool bacs_ppzksnark_proving_key<ppT>::operator==(
    const bacs_ppzksnark_proving_key<ppT> &other) const
{
    ...
    const bacs_ppzksnark_circuit<ppT> &circuit)
{
    ...
    const r1cs_constraint_system<FieldT> r1cs_cs =
        bacs_to_r1cs_instance_map<FieldT>(circuit);
    const r1cs_ppzksnark_keypair<ppT> r1cs_keypair =
        r1cs_ppzksnark_generator<ppT>(r1cs_cs);
    libff::leave_block("Call to bacs_ppzksnark_generator");

    return bacs_ppzksnark_keypair<ppT>(
        bacs_ppzksnark_proving_key<ppT>(circuit, r1cs_keypair.pk),
        r1cs_keypair.vk);
}
    ...
    const bacs_ppzksnark_auxiliary_input<ppT> &auxiliary_input)
{
    ...
    const r1cs_variable_assignment<FieldT> r1cs_va =
        bacs_to_r1cs_witness_map<FieldT>(
            pk.circuit, primary_input, auxiliary_input);
    const r1cs_auxiliary_input<FieldT> r1cs_ai(
        r1cs_va.begin() + primary_input.size(),
        r1cs_va.end()); // TODO: faster to just change bacs_to_r1cs_witness_map
                        // into two :(
    ...
    const bacs_ppzksnark_proof<ppT> &proof)
{
    ...
    const bacs_ppzksnark_processed_verification_key<ppT> pvk =
        bacs_ppzksnark_verifier_process_vk<ppT>(vk);
    const bool bit = r1cs_ppzksnark_online_verifier_strong_IC<ppT>(
