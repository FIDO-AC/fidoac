...
void ALU_jmp_gadget<FieldT>::generate_r1cs_constraints()
{
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>({ONE}, {this->argval2.packed}, {this->result}),
        FMT(this->annotation_prefix, " jmp_result"));
}

template<typename FieldT> void ALU_jmp_gadget<FieldT>::generate_r1cs_witness()
{
    this->pb.val(this->result) = this->pb.val(this->argval2.packed);
}

template<typename FieldT> void test_ALU_jmp_gadget()
{
    ...
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(
            ...
            pb_packing_sum<FieldT>(pb_variable_array<FieldT>(
                this->argval2.bits.begin() + this->pb.ap.subaddr_len(),
                this->argval2.bits.end())) -
                this->pc.packed - 1,
            this->result - this->pc.packed - 1),
        FMT(this->annotation_prefix, " cjmp_result"));
}

template<typename FieldT> void ALU_cjmp_gadget<FieldT>::generate_r1cs_witness()
{
    this->pb.val(this->result) =
        ((this->pb.val(this->flag) == FieldT::one())
             ? FieldT(
                   this->pb.val(this->argval2.packed).as_ulong() >>
                   this->pb.ap.subaddr_len())
             : this->pb.val(this->pc.packed) + FieldT::one());
}

template<typename FieldT> void test_ALU_cjmp_gadget()
{
    // TODO: update
...
template<typename FieldT> void ALU_cnjmp_gadget<FieldT>::generate_r1cs_witness()
{
    this->pb.val(this->result) =
        ((this->pb.val(this->flag) == FieldT::one())
             ? this->pb.val(this->pc.packed) + FieldT::one()
...
template<typename FieldT> void test_ALU_cnjmp_gadget()
{
    // TODO: update
