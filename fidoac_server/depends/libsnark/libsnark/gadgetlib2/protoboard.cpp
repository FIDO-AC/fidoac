/** @file
 *****************************************************************************
 Implementation of Protoboard, a "memory manager" for building arithmetic
 constraints
 *****************************************************************************
...
void Protoboard::setValuesAsBitArray(
    const VariableArray &varArray, const size_t srcValue)
{
    GADGETLIB_ASSERT(
...
void Protoboard::setDualWordValue(
    const DualWord &dualWord, const size_t srcValue)
{
    setMultipackedWordValue(dualWord.multipacked(), srcValue);
    setValuesAsBitArray(dualWord.unpacked(), srcValue);
}

void Protoboard::setMultipackedWordValue(
    const MultiPackedWord &multipackedWord, const size_t srcValue)
{
    if (fieldType_ == R1P) {
        GADGETLIB_ASSERT(
...
// The following 3 methods are purposely not overloaded to the same name in
// order to reduce programmer error. We want the programmer to explicitly code
// what type of constraint she wants.
void Protoboard::addRank1Constraint(
...
void Protoboard::addGeneralConstraint(
    const Polynomial &a, const Polynomial &b, const ::std::string &name)
{
    constraintSystem_.addConstraint(PolynomialConstraint(a, b, name));
}

void Protoboard::addUnaryConstraint(
...
bool multipackedAndUnpackedValuesDisagree(
    const bool multipackedEqualsValue, const bool unpackedEqualsValue)
{
    return multipackedEqualsValue != unpackedEqualsValue;
}

void printInformativeNoticeMessage(
    const bool multipackedEqualsValue, const bool unpackedEqualsValue)
{
    if (multipackedEqualsValue == true && unpackedEqualsValue == false) {
        cout << "NOTE: multipacked value equals expected value but unpacked "
                "value does not!"
             << endl;
    } else {
        GADGETLIB_ASSERT(
            multipackedEqualsValue == false && unpackedEqualsValue == true,
            "printInformativeNoticeMessage(...) has been called incorrectly");
        cout << "NOTE: unpacked value equals expected value but multipacked "
                "value does not!"
             << endl;
    }
}

bool Protoboard::dualWordAssignmentEqualsValue(
...
bool expectedToPrintValues(
    const bool boolValue, const PrintOptions &printOption)
{
    return (
        (boolValue == true && printOption == PrintOptions::DBG_PRINT_IF_TRUE) ||
        (boolValue == false &&
         printOption == PrintOptions::DBG_PRINT_IF_FALSE));
}

bool Protoboard::multipackedWordAssignmentEqualsValue(
