...
/// An interface class for field elements. Currently 2 classes will derive from
/// this interface: R1P_Elem - Elements of a field of prime characteristic
/// FConst - Formally not a field, only placeholders for field agnostic
/// constants, such as 0 and 1. Can be used for -1 or any other constant which
/// makes semantic sense in all fields.
class FElemInterface
{
public:
    virtual FElemInterface &operator=(const long n) = 0;
    /// FConst will be field agnostic, allowing us to hold values such as 0 and
    /// 1 without knowing the underlying field. This assignment operator will
    /// convert to the correct field element.
    ...
    /// This operator is not always mathematically well defined. 'n' will be
    /// checked in runtime for fields in which integer values are not well
    /// defined.
...
inline bool operator!=(const FElemInterface &first, const long second)
{
    return !(first == second);
}
inline bool operator!=(
    const FElemInterface &first, const FElemInterface &second)
{
    return !(first == second);
}

/// A wrapper class for field elements. Can hold any derived type of
...
public:
    ...
    /// Helper method. When doing arithmetic between a constant and a field
    /// specific element we want to "promote" the constant to the same field.
    /// This function changes the unique_ptr to point to a field specific
    /// element with the same value as the constant which it held.
    ...
    inline friend ::std::ostream &operator<<(
        ::std::ostream &os, const FElem &elem)
    {
...
inline bool operator!=(const FElem &first, const FElem &second)
{
    return !(first == second);
}

/// These operators are not always mathematically well defined. The long will be
/// checked in runtime for fields in which values other than 0 and 1 are not
/// well defined.
inline bool operator==(const FElem &first, const long second)
{
    return first == FElem(second);
}
inline bool operator==(const long first, const FElem &second)
{
    return second == first;
}
inline bool operator!=(const FElem &first, const long second)
...
/// A field agnostic constant. All fields have constants 1 and 0 and this class
/// allows us to hold an element agnostically while the context field is not
/// known. For example, when given the very useful expression '1 - x' where x
/// is a field agnostic formal variable, we must store the constant '1' without
/// knowing over which field this expression will be evaluated. Constants can
/// also hold integer values, which will be evaluated if possible, in runtime.
/// For instance the expression '42 + x' will be evaluated in runtime in the
/// trivial way when working over the prime characteristic Galois Field GF_43
/// but will cause a runtime error when evaluated over a GF2 extension field in
/// which '42' has no obvious meaning, other than being the answer to life, the
...
public:
    FConst(const FConst &src) : contents_(src.contents_) {}
    ...
    {
        return GADGETLIB2_FMT("%ld", contents_);
    }
    ...
    {
        return FElemInterfacePtr(new FConst(*this));
    }
    /// @return a unique_ptr to a new copy of the element's multiplicative
    /// inverse
    ...
    {
        libff::UNUSED(i);
        GADGETLIB_FATAL("Cannot get bit from FConst.");
    }
...
/// Holds elements of a prime characteristic field. Currently implemented using
/// the gmp (Linux) and mpir (Windows) libraries.
...
///
/// Each variable is specified by an index. This can be imagined as the index
/// in x_1, x_2,..., x_i These are formal variables and do not hold an
/// assignment, later the class VariableAssignment will give each formal
/// variable its own assignment. Variables have no comparison and assignment
/// operators as evaluating (x_1 == x_2) has no sense without specific
/// assignments. Variables are field agnostic, this means they can be used
/// regardless of the context field, which will also be determined by the
/// assignment.
class Variable
{
private:
    VarIndex_t index_; ///< This index differentiates and identifies Variable
                       ///< instances.
    static VarIndex_t nextFreeIndex_; ///< Monotonically-increasing counter to
                                      ///< allocate disinct indices.
#ifdef DEBUG
    ::std::string name_;
#endif
...
public:
    ...
    /// This is not an ordering of Variable assignments and has no semantic
    /// meaning.
    ...
    typedef ::std::map<Variable, FElem, Variable::VariableStrictOrder>
        VariableAssignment;
...
/// Holds variable whose purpose is to be populated with a boolean value,
/// Field(0) or Field(1)
typedef Variable FlagVariable;
typedef VariableArray FlagVariableArray;

/// Represents a packed word that can fit in a field element. For a word
/// representing an unsigned integer for instance this means we require (int <
/// fieldSize)
typedef Variable PackedWord;
typedef VariableArray PackedWordArray;

/// Holds variables whose purpose is to be populated with the unpacked form of
/// some word, bit by bit
class UnpackedWord : public VariableArray
{
public:
    UnpackedWord() : VariableArray() {}
    UnpackedWord(const size_t numBits, const ::std::string &name)
        : VariableArray(numBits, name)
    {
    }
}; // class UnpackedWord
...
/// Holds variables whose purpose is to be populated with the packed form of
/// some word. word representation can be larger than a single field element in
/// small enough fields
...
{
private:
    // kept as 2 separate arrays because the more common usecase will be to
...
public:
    ...
    // For cases in which we can assume each unpacked value fits < in 1 packed
    // Variable
...
public:
    ...
    LinearCombination(const LinearTerm &linTerm)
        : linearTerms_(1, linTerm), constant_(0)
    {
    }
...
inline LinearCombination operator-(const LinearCombination &lc)
{
    return LinearCombination(0) -= lc;
}
...
private:
    FElem coeff_;
    // currently just a vector of variables. This can surely be optimized e.g.
    // hold a variable-degree pair but is not needed for concrete efficiency as
    // we will only be invoking degree 2 constraints in the near future.
    Variable::multiset variables_;
...
public:
    ...
    Polynomial(const LinearTerm &linearTerm)
        : monomials_(1, Monomial(linearTerm)), constant_(0)
    {
    }
    ...
    {
        return *this += Polynomial(Monomial(other));
    }
}; // class Polynomial

inline Polynomial operator-(const Polynomial &src)
{
    return Polynomial(FElem(0)) -= src;
}
