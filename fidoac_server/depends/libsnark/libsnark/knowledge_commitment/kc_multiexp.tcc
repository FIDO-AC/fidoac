...
knowledge_commitment<T1, T2> opt_window_wnaf_exp(
    ...
    const size_t chunks)
{
    ...
    auto index_it =
        std::lower_bound(vec.indices.begin(), vec.indices.end(), min_idx);
    ...
    return acc + libff::multi_exp<
                     knowledge_commitment<T1, T2>,
                     FieldT,
                     Method,
                     BaseForm>(g.begin(), g.end(), p.begin(), p.end(), chunks);
}

template<typename T1, typename T2, typename FieldT>
knowledge_commitment_vector<T1, T2> kc_batch_exp_internal(
    ...
    for (size_t pos = start_pos; pos != end_pos; ++pos) {
        if (!v[pos].is_zero()) {
            res.values.emplace_back(knowledge_commitment<T1, T2>(
                windowed_exp(
                    scalar_size, T1_window, T1_table, T1_coeff * v[pos]),
                windowed_exp(
                    scalar_size, T2_window, T2_table, T2_coeff * v[pos])));
...
template<typename T1, typename T2, typename FieldT>
knowledge_commitment_vector<T1, T2> kc_batch_exp(
    const size_t scalar_size,
    const size_t T1_window,
    const size_t T2_window,
    const libff::window_table<T1> &T1_table,
    ...
    bool output_special)
{
    ...
    const size_t num_chunks =
        std::max((size_t)1, std::min(nonzero, suggested_num_chunks));

    if (!libff::inhibit_profiling_info) {
        libff::print_indent();
        printf(
            "Non-zero coordinate count: %zu/%zu (%0.2f%%)\n",
            nonzero,
            v.size(),
            100. * nonzero / v.size());
    }
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < num_chunks; ++i) {
        tmp[i] = kc_batch_exp_internal<T1, T2, FieldT>(
            scalar_size,
            T1_window,
            T2_window,
            T1_table,
            ...
            chunk_pos[i],
            chunk_pos[i + 1],
            i == num_chunks - 1 ? last_chunk : chunk_size);
        if (output_special) {
            libff::batch_to_special<knowledge_commitment<T1, T2>>(
                tmp[i].values);
        }
    }
    ...
    if (num_chunks == 1) {
        ...
        for (size_t i = 0; i < num_chunks; ++i) {
            res.values.insert(
                res.values.end(), tmp[i].values.begin(), tmp[i].values.end());
            res.indices.insert(
                res.indices.end(),
                tmp[i].indices.begin(),
                tmp[i].indices.end());
