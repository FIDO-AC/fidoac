...
digest_selector_gadget<FieldT>::digest_selector_gadget(
        ...
        */
        this->pb.add_r1cs_constraint(
            r1cs_constraint<FieldT>(
                is_right,
                right.bits[i] - left.bits[i],
                input.bits[i] - left.bits[i]),
...
void digest_selector_gadget<FieldT>::generate_r1cs_witness()
{
    ...
    assert(
        this->pb.lc_val(is_right) == FieldT::one() ||
        this->pb.lc_val(is_right) == FieldT::zero());
