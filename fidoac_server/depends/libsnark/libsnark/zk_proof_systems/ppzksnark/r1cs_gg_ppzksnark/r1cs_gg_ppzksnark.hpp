...
- ppzkSNARK = "PreProcessing Zero-Knowledge Succinct Non-interactive ARgument of
Knowledge"
...
Model" Sean Bowe and Ariel Gabizon and Ian Miers, IACR Cryptology ePrint Archive
2017, <http://eprint.iacr.org/2017/1050>
...
template<typename ppT> class r1cs_gg_ppzksnark_proving_key;
...
public:
    ...
    r1cs_gg_ppzksnark_proving_key<ppT> &operator=(
        const r1cs_gg_ppzksnark_proving_key<ppT> &other) = default;
    r1cs_gg_ppzksnark_proving_key(
        const r1cs_gg_ppzksnark_proving_key<ppT> &other) = default;
    r1cs_gg_ppzksnark_proving_key(r1cs_gg_ppzksnark_proving_key<ppT> &&other) =
        default;
    r1cs_gg_ppzksnark_proving_key(
        libff::G1<ppT> &&alpha_g1,
        libff::G1<ppT> &&beta_g1,
    ...
    {
        // 1 (alpha_g1) + 1 (beta_g1) + 1 (delta_g1) + A_query + B_query +
        // H_query + L_query
        return 1 + 1 + 1 + A_query.size() + B_query.domain_size() +
               H_query.size() + L_query.size();
    }
    ...
    {
        return 1 + A_query.size() + B_query.size() + H_query.size() +
               L_query.size();
    }
    ...
    {
        return (
            libff::size_in_bits(A_query) + B_query.size_in_bits() +
            libff::size_in_bits(H_query) + libff::size_in_bits(L_query) +
            1 * libff::G1<ppT>::size_in_bits() +
            1 * libff::G2<ppT>::size_in_bits());
    }
    ...
    friend std::istream &operator>>
        <ppT>(std::istream &in, r1cs_gg_ppzksnark_proving_key<ppT> &pk);
};

/******************************* Verification key ****************************/

template<typename ppT> class r1cs_gg_ppzksnark_verification_key;
...
public:
    ...
    size_t G1_size() const { return ABC_g1.size() + 1; }
    ...
    size_t size_in_bits() const
    {
        ...
        return (
            ABC_g1.size_in_bits() + 2 * libff::G2<ppT>::size_in_bits() +
    ...
    friend std::ostream &operator<<<ppT>(
        std::ostream &out, const r1cs_gg_ppzksnark_verification_key<ppT> &vk);
    friend std::istream &operator>>
        <ppT>(std::istream &in, r1cs_gg_ppzksnark_verification_key<ppT> &vk);

    static r1cs_gg_ppzksnark_verification_key<ppT> dummy_verification_key(
        const size_t input_size);
};

/************************ Processed verification key *************************/

template<typename ppT> class r1cs_gg_ppzksnark_processed_verification_key;
...
public:
    ...
    bool operator==(
        const r1cs_gg_ppzksnark_processed_verification_key &other) const;
    friend std::ostream &operator<<<ppT>(
        std::ostream &out,
        const r1cs_gg_ppzksnark_processed_verification_key<ppT> &pvk);
...
/**
 * A key pair for the R1CS GG-ppzkSNARK, which consists of a proving key and a
 * verification key.
 */
template<typename ppT> class r1cs_gg_ppzksnark_keypair
{
public:
    ...
    r1cs_gg_ppzksnark_keypair(const r1cs_gg_ppzksnark_keypair<ppT> &other) =
        default;
...
template<typename ppT> class r1cs_gg_ppzksnark_proof;
...
public:
    ...
    r1cs_gg_ppzksnark_proof(
        libff::G1<ppT> &&g_A, libff::G2<ppT> &&g_B, libff::G1<ppT> &&g_C)
    ...
    {
        return (
            g_A.is_well_formed() && g_B.is_well_formed() &&
            g_C.is_well_formed());
    }
    ...
    friend std::ostream &operator<<<ppT>(
        std::ostream &out, const r1cs_gg_ppzksnark_proof<ppT> &proof);
    friend std::istream &operator>>
        <ppT>(std::istream &in, r1cs_gg_ppzksnark_proof<ppT> &proof);
};
...
/**
 * For debugging purposes (of
 * r1cs_gg_ppzksnark_r1cs_gg_ppzksnark_verifier_gadget):
 *
 * A verifier algorithm for the R1CS GG-ppzkSNARK that:
