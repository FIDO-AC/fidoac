/** @file
 *****************************************************************************
 Declarations of the interfaces and basic gadgets for R1P (Rank 1 prime
 characteristic) constraint systems.
...
void Gadget::addUnaryConstraint(
    const LinearCombination &a, const ::std::string &name)
{
    pb_->addUnaryConstraint(a, name);
}

void Gadget::addRank1Constraint(
...
void R1P_Gadget::addRank1Constraint(
    const LinearCombination &a,
    const LinearCombination &b,
    const LinearCombination &c,
    const string &name)
...
*/
BinaryAND_Gadget::BinaryAND_Gadget(
    ProtoboardPtr pb,
...
{
    addRank1Constraint(
        input1_, input2_, result_, "result = AND(input1, input2)");
}
...
R1P_AND_Gadget::R1P_AND_Gadget(
    ProtoboardPtr pb, const VariableArray &input, const Variable &result)
    : Gadget(pb)
    , AND_GadgetBase(pb)
    , R1P_Gadget(pb)
    , input_(input)
    ...
    , sumInverse_("sumInverse")
{
    GADGETLIB_ASSERT(
        input.size() > 0,
        "Attempted to create an R1P_AND_Gadget with 0 inputs.");
    GADGETLIB_ASSERT(
        input.size() <= Fp(-1).as_ulong(),
        "Attempted to create R1P_AND_Gadget with too "
...
{
    addRank1Constraint(
        sum_, result_, 0, "sum * result = 0 | sum == sum(input[i]) - n");
    addRank1Constraint(
        sumInverse_,
        sum_,
        1 - result_,
...
*/
BinaryOR_Gadget::BinaryOR_Gadget(
    ProtoboardPtr pb,
    const LinearCombination &input1,
    const LinearCombination &input2,
    const Variable &result)
    ...
    , result_(result)
{
    GADGETLIB_ASSERT(
        input.size() > 0,
        "Attempted to create an R1P_OR_Gadget with 0 inputs.");
    GADGETLIB_ASSERT(
        input.size() <= Fp(-1).as_ulong(),
        "Attempted to create R1P_OR_Gadget with too "
        "many inputs. Will cause overflow!");
}

void R1P_OR_Gadget::init() { sum_ = sum(input_); }

void R1P_OR_Gadget::generateConstraints()
{
    addRank1Constraint(
        sum_, 1 - result_, 0, "sum * (1 - result) = 0 | sum == sum(input[i])");
    addRank1Constraint(
        sumInverse_,
        sum_,
        result_,
...
R1P_InnerProduct_Gadget::R1P_InnerProduct_Gadget(
    ProtoboardPtr pb,
    const VariableArray &A,
    const VariableArray &B,
    const Variable &result)
    : Gadget(pb)
    , InnerProduct_GadgetBase(pb)
    , R1P_Gadget(pb)
    , partialSums_(A.size(), "partialSums")
    , A_(A)
    , B_(B)
    , result_(result)
{
    GADGETLIB_ASSERT(
        A.size() > 0,
        "Attempted to create an R1P_InnerProduct_Gadget with 0 inputs.");
    GADGETLIB_ASSERT(
        A.size() == B.size(),
        GADGETLIB2_FMT(
            "Inner product vector sizes not equal. Sizes are: "
            "(A) - %u, (B) - %u",
            A.size(),
            B.size()));
}
...
void R1P_InnerProduct_Gadget::generateConstraints()
{
    ...
    // else (n > 1)
    addRank1Constraint(
        A_[0], B_[0], partialSums_[0], "A[0] * B[0] = partialSums[0]");
    for (int i = 1; i <= n - 2; ++i) {
        addRank1Constraint(
            A_[i],
            B_[i],
...
void R1P_InnerProduct_Gadget::generateWitness()
{
    ...
    for (int i = 1; i <= n - 2; ++i) {
        val(partialSums_[i]) =
            val(partialSums_[i - 1]) + val(A_[i]) * val(B_[i]);
    }
...
/*
    Constraint breakdown:
    (1) indicators[i] * (index - i) = 0  | i = 0..n-1 ==> only indicators[index]
   will be non-zero (2) sum(indicators[i]) = successFlag ==> successFlag =
   indicators[index] (3) successFlag is boolean (4) result[j] = <indicators> *
   <inputs[index][j]>  |  j = 1..output.size()   ==> result[j] =
   inputs[index][j]

*/

R1P_LooseMUX_Gadget::R1P_LooseMUX_Gadget(
    ProtoboardPtr pb,
    ...
    : Gadget(pb)
    , LooseMUX_GadgetBase(pb)
    , R1P_Gadget(pb)
    , indicators_(inputs.size(), "indicators")
    , inputs_(inputs.size())
    , index_(index)
    , output_(output)
...
{
    GADGETLIB_ASSERT(
        inputs.size() <= Fp(-1).as_ulong(),
        "Attempted to create R1P_LooseMUX_Gadget "
    ...
    ::std::copy(
        inputs.begin(),
        inputs.end(),
        inputs_.begin()); // change type to R1P_VariableArray
}

void R1P_LooseMUX_Gadget::init()
{
    // create inputs for the inner products and initialize them. Each iteration
    // creates a VariableArray for the i'th elements from each of the vector's
    // VariableArrays.
    for (size_t i = 0; i < output_.size(); ++i) {
        ...
        computeResult_.push_back(InnerProduct_Gadget::create(
            pb_, indicators_, curInput, output_[i]));
    }
}
...
void R1P_LooseMUX_Gadget::generateConstraints()
{
    ...
    addRank1Constraint(
        sum(indicators_), 1, successFlag_, "sum(indicators) * 1 = successFlag");
...
VariableArray R1P_LooseMUX_Gadget::indicatorVariables() const
{
    return indicators_;
}

GadgetPtr LooseMUX_Gadget::create(
    ProtoboardPtr pb,
    const MultiPackedWordArray &inputs,
    const Variable &index,
    const VariableArray &output,
    const Variable &successFlag)
{
    ...
    if (pb->fieldType_ == R1P) {
        pGadget.reset(
            new R1P_LooseMUX_Gadget(pb, inputs, index, output, successFlag));
    } else {
        GADGETLIB_FATAL(
            "Attempted to create gadget of undefined Protoboard type.");
    }
    ...
    const Variable &successFlag)
{
    ...
    auto result =
        LooseMUX_Gadget::create(pb, inpVec, index, outVec, successFlag);
    return result;
}
...
/*
    Compression Packing gadgets have two modes, which differ in the way the
   witness and constraints are created. In PACK mode  gerateWitness() will take
   the bits and create a packed element (or number of elements) while
   generateConstraints() will not enforce that bits are indeed Boolean. In
   UNPACK mode generateWitness() will take the packed representation and unpack
   it to bits while generateConstraints will in addition enforce that unpacked
   bits are indeed Boolean.
*/
...
R1P_CompressionPacking_Gadget::R1P_CompressionPacking_Gadget(
    ProtoboardPtr pb,
    const VariableArray &unpacked,
    const VariableArray &packed,
    PackingMode packingMode)
    : Gadget(pb)
    , CompressionPacking_GadgetBase(pb)
    , R1P_Gadget(pb)
    ...
    , packed_(packed)
{
    ...
    GADGETLIB_ASSERT(
        packed.size() == 1,
        "Attempted to pack into more than 1 Variable in "
...
void R1P_CompressionPacking_Gadget::generateConstraints()
{
    ...
    addRank1Constraint(
        packed_[0], 1, packed, "packed[0] = sum(2^i * unpacked[i])");
}
...
void R1P_CompressionPacking_Gadget::generateWitness()
{
    ...
    if (packingMode_ == PackingMode::PACK) {
        ...
        for (int i = 0; i < n; ++i) {
            GADGETLIB_ASSERT(
                val(unpacked_[i]).asLong() == 0 ||
                    val(unpacked_[i]).asLong() == 1,
                GADGETLIB2_FMT(
...
/*
    Arithmetic Packing gadgets have two modes, which differ in the way the
   witness and constraints are created. In PACK mode  gerateWitness() will take
   the bits and create a packed element (or number of elements) while
   generateConstraints() will not enforce that bits are indeed Boolean. In
   UNPACK mode generateWitness() will take the packed representation and unpack
...
R1P_IntegerPacking_Gadget::R1P_IntegerPacking_Gadget(
    ProtoboardPtr pb,
    const VariableArray &unpacked,
    const VariableArray &packed,
    PackingMode packingMode)
    : Gadget(pb)
    , IntegerPacking_GadgetBase(pb)
    , R1P_Gadget(pb)
    , packingMode_(packingMode)
    , unpacked_(unpacked)
    , packed_(packed)
{
    ...
    GADGETLIB_ASSERT(
        packed.size() == 1,
        "Attempted to pack into more than 1 Variable in "
        "R1P_IntegerPacking_Gadget.")
}

void R1P_IntegerPacking_Gadget::init()
{
    compressionPackingGadget_ = CompressionPacking_Gadget::create(
        pb_, unpacked_, packed_, packingMode_);
}
...
R1P_EqualsConst_Gadget::R1P_EqualsConst_Gadget(
    ProtoboardPtr pb,
    const FElem &n,
    const LinearCombination &input,
    const Variable &result)
    : Gadget(pb)
    , EqualsConst_GadgetBase(pb)
    , R1P_Gadget(pb)
    , n_(n)
    , aux_("aux (R1P_EqualsConst_Gadget)")
    , input_(input)
    , result_(result)
{
}
...
void R1P_EqualsConst_Gadget::generateConstraints()
{
    ...
    addRank1Constraint(
        input_ - n_, aux_, 1 - result_, "(input - n) * aux = 1 - result");
}
...
/*************************************************************************************************/
/*************************************************************************************************/
DualWord_Gadget::DualWord_Gadget(
    ProtoboardPtr pb, const DualWord &var, PackingMode packingMode)
...
void DualWord_Gadget::init()
{
    packingGadget_ = CompressionPacking_Gadget::create(
        pb_, var_.unpacked(), var_.multipacked(), packingMode_);
}

GadgetPtr DualWord_Gadget::create(
    ProtoboardPtr pb, const DualWord &var, PackingMode packingMode)
{
...
void DualWord_Gadget::generateWitness() { packingGadget_->generateWitness(); }
...
/*************************************************************************************************/
/*************************************************************************************************/
DualWordArray_Gadget::DualWordArray_Gadget(
    ProtoboardPtr pb, const DualWordArray &vars, PackingMode packingMode)
...
void DualWordArray_Gadget::init()
{
    ...
    for (size_t i = 0; i < vars_.size(); ++i) {
        const auto curGadget = CompressionPacking_Gadget::create(
            pb_, unpacked[i], packed[i], packingMode_);
        packingGadgets_.push_back(curGadget);
    }
}

GadgetPtr DualWordArray_Gadget::create(
    ProtoboardPtr pb, const DualWordArray &vars, PackingMode packingMode)
{
...
GadgetPtr Toggle_Gadget::create(
    ProtoboardPtr pb,
    const FlagVariable &toggle,
    const LinearCombination &zeroValue,
    const LinearCombination &oneValue,
...
{
    GadgetPtr pGadget(
        new Toggle_Gadget(pb, toggle, zeroValue, oneValue, result));
    pGadget->init();
    return pGadget;
}
...
ConditionalFlag_Gadget::ConditionalFlag_Gadget(
    ProtoboardPtr pb,
...
GadgetPtr ConditionalFlag_Gadget::create(
    ProtoboardPtr pb,
    const LinearCombination &condition,
    const FlagVariable &flag)
{
...
{
    pb_->addRank1Constraint(
        condition_, negate(flag_), 0, "condition * not(flag) = 0");
    pb_->addRank1Constraint(
        condition_,
...
LogicImplication_Gadget::LogicImplication_Gadget(
    ProtoboardPtr pb,
    const LinearCombination &condition,
    const FlagVariable &flag)
...
GadgetPtr LogicImplication_Gadget::create(
    ProtoboardPtr pb,
    const LinearCombination &condition,
    const FlagVariable &flag)
{
...
R1P_Comparison_Gadget::R1P_Comparison_Gadget(
    ProtoboardPtr pb,
    ...
    , alpha_u_(wordBitSize, "alpha")
    , notAllZeroes_("notAllZeroes")
{
}
...
void R1P_Comparison_Gadget::init()
{
    ...
    alphaDualVariablePacker_ = CompressionPacking_Gadget::create(
        pb_, alpha_u_, VariableArray(1, alpha_p_), PackingMode::UNPACK);
}
/*
    Constraint breakdown:
...
void R1P_Comparison_Gadget::generateConstraints()
{
    ...
    addRank1Constraint(
        1, alpha_p_, two_n + rhs_ - lhs_, "packed(alpha) = 2^n + B - A");
    alphaDualVariablePacker_->generateConstraints();
    allZeroesTest_->generateConstraints();
    addRank1Constraint(
        1, alpha_u_[wordBitSize_], lessOrEqual_, "alpha[n] = lessOrEqual");
    addRank1Constraint(
