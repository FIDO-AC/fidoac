...
fooram_cpu_checker<FieldT>::fooram_cpu_checker(
...
{
    /* increment PC */
    packed_next_pc_addr.allocate(
        pb, FMT(annotation_prefix, " packed_next_pc_addr"));
    pack_next_pc_addr.reset(new packing_gadget<FieldT>(
        pb,
        next_pc_addr,
        packed_next_pc_addr,
        FMT(annotation_prefix, " pack_next_pc_addr")));
    ...
    increment_pc.reset(new bar_gadget<FieldT>(
        pb,
        prev_pc_addr,
        FieldT::one(),
        one_as_addr,
        FieldT::one(),
        packed_next_pc_addr,
        FMT(annotation_prefix, " increment_pc")));

    /* packed_store_addr = prev_pc_addr + prev_pc_val */
    packed_store_addr.allocate(
        pb, FMT(annotation_prefix, " packed_store_addr"));
    compute_packed_store_addr.reset(new bar_gadget<FieldT>(
        pb,
        prev_pc_addr,
        FieldT::one(),
        prev_pc_val,
    ...
    compute_packed_load_addr.reset(new bar_gadget<FieldT>(
        ...
        FieldT::one(),
        packed_load_addr,
        FMT(annotation_prefix, " compute_packed_load_addr")));
    ...
    */
    ...
    pack_ls_addr.reset(new packing_gadget<FieldT>(
        pb, ls_addr, packed_ls_addr, " pack_ls_addr"));

    /* packed_store_val = prev_state_bits + prev_pc_addr */
    packed_store_val.allocate(pb, FMT(annotation_prefix, " packed_store_val"));
    compute_packed_store_val.reset(new bar_gadget<FieldT>(
        pb,
        prev_state,
        FieldT::one(),
        prev_pc_addr,
        FieldT::one(),
        packed_store_val,
        FMT(annotation_prefix, " compute_packed_store_val")));

    /*
      packed_ls_next_val = x0 * packed_ls_prev_val + (1-x0) * packed_store_val
      packed_ls_next_val ~ ls_next_val
    */
    packed_ls_prev_val.allocate(
        pb, FMT(annotation_prefix, " packed_ls_prev_val"));
    pack_ls_prev_val.reset(new packing_gadget<FieldT>(
        this->pb,
        ls_prev_val,
        packed_ls_prev_val,
        FMT(annotation_prefix, " pack_ls_prev_val")));
    packed_ls_next_val.allocate(
        pb, FMT(annotation_prefix, " packed_ls_next_val"));
    pack_ls_next_val.reset(new packing_gadget<FieldT>(
        this->pb,
        ls_next_val,
        packed_ls_next_val,
        FMT(annotation_prefix, " pack_ls_next_val")));
    ...
    */
    packed_prev_state.allocate(
        pb, FMT(annotation_prefix, " packed_prev_state"));
    pack_prev_state.reset(new packing_gadget<FieldT>(
        pb, prev_state, packed_prev_state, " pack_prev_state"));

    packed_next_state.allocate(
        pb, FMT(annotation_prefix, " packed_next_state"));
    pack_next_state.reset(new packing_gadget<FieldT>(
        pb, next_state, packed_next_state, " pack_next_state"));
...
void fooram_cpu_checker<FieldT>::generate_r1cs_constraints()
{
    ...
    /*
      packed_ls_addr = x0 * packed_load_addr + (1-x0) * packed_store_addr
      packed_ls_addr - packed_store_addr = x0 * (packed_load_addr -
      packed_store_addr) packed_ls_addr ~ ls_addr
    */
    ...
    /*
      ...
      packed_store_val) packed_ls_next_val ~ ls_next_val
    */
    ...
    /*
      packed_next_state = x0 * packed_ls_prev_val + (1-x0) * packed_prev_state
      packed_next_state - packed_prev_state = x0 * (packed_ls_prev_val -
      packed_prev_state) packed_next_state ~ next_state packed_prev_state ~
      prev_state
    */
    ...
    /* next_has_accepted = 1 */
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(1, next_has_accepted, 1),
        FMT(this->annotation_prefix, " always_accepted"));
}
    ...
    */
    this->pb.val(packed_ls_addr) =
        (this->pb.val(prev_pc_val[0]) * this->pb.val(packed_load_addr) +
         (FieldT::one() - this->pb.val(prev_pc_val[0])) *
             this->pb.val(packed_store_addr));
    pack_ls_addr->generate_r1cs_witness_from_packed();
}

template<typename FieldT>
void fooram_cpu_checker<FieldT>::generate_r1cs_witness_other(
...
template<typename FieldT> void fooram_cpu_checker<FieldT>::dump() const
{
