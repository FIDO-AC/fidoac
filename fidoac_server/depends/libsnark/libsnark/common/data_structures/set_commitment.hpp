...
struct set_membership_proof {
    ...
    friend std::ostream &operator<<(
        std::ostream &out, const set_membership_proof &other);
    friend std::istream &operator>>(
        std::istream &in, set_membership_proof &other);
};

template<typename HashT> class set_commitment_accumulator
{
private:
...
public:
    ...
    set_commitment_accumulator(
        const size_t max_entries, const size_t value_size = 0);
    ...
    set_membership_proof get_membership_proof(
