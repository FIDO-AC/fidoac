...
TEST(gadgetLib2, VariableStrictOrdering)
{
    ...
    EXPECT_TRUE(
        orderFunc(v1, v2) || orderFunc(v2, v1)); // check strict ordering
    v2 = v1;
...
TEST(gadgetLib2, VariableArray)
{
    ...
    Variable::VariableStrictOrder orderFunc;
    EXPECT_TRUE(
        orderFunc(vArr[0], vArr[1]) ||
        orderFunc(vArr[1], vArr[0])); // check strict ordering
    vArr[1] = vArr[0];
    EXPECT_FALSE(
        orderFunc(vArr[0], vArr[1]) ||
        orderFunc(vArr[1], vArr[0])); // check strict ordering
...
TEST(gadgetLib2, FElem_FConst_operatorEquals)
{
    ...
    FElem e2(FElem(2));
    e0 = e1 = 42;
    // bool operator==(const FElem& other) const {return *elem_ ==
    // *other.elem_;}
    EXPECT_TRUE(e1 == e0);
    EXPECT_FALSE(e1 == e2);
    FElem eR1P = Fp(42);
...
{
    initPublicParamsFromDefaultPp();
    // FElem& operator+=(const FElem& other) {*elem_ += *other.elem_; return
    // *this;}
    FElem e0(0);
    FElem e1(0);
    e0 = e1 = 42;
...
TEST(gadgetLib2, LinearTermConstructors)
{
    ...
    // LinearTerm(const Variable& v, const FElem& coeff) : variable_(v),
    // coeff_(coeff) {}
...
TEST(gadgetLib2, LinearCombination)
{
    ...
    //    LinearCombination(const Variable& var) : linearTerms_(1,var),
    //    constant_(0) {}
    VariableArray x(10, "x");
...
TEST(gadgetLib2, MonomialConstructors)
{
    ...
    // Monomial(const Variable& var, const FElem& coeff) : coeff_(coeff),
    // variables_(1, var) {}
