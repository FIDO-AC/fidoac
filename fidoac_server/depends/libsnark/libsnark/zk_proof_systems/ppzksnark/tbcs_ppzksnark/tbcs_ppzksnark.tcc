...
bool tbcs_ppzksnark_proving_key<ppT>::operator==(
    const tbcs_ppzksnark_proving_key<ppT> &other) const
{
    ...
    const tbcs_ppzksnark_circuit &circuit)
{
    ...
    const uscs_constraint_system<FieldT> uscs_cs =
        tbcs_to_uscs_instance_map<FieldT>(circuit);
    const uscs_ppzksnark_keypair<ppT> uscs_keypair =
        uscs_ppzksnark_generator<ppT>(uscs_cs);
    libff::leave_block("Call to tbcs_ppzksnark_generator");

    return tbcs_ppzksnark_keypair<ppT>(
    ...
    const tbcs_ppzksnark_auxiliary_input &auxiliary_input)
{
    ...
    const uscs_variable_assignment<FieldT> uscs_va =
        tbcs_to_uscs_witness_map<FieldT>(
            pk.circuit, primary_input, auxiliary_input);
    const uscs_primary_input<FieldT> uscs_pi =
        libff::convert_bit_vector_to_field_element_vector<FieldT>(
            primary_input);
    const uscs_auxiliary_input<FieldT> uscs_ai(
        uscs_va.begin() + primary_input.size(),
        uscs_va.end()); // TODO: faster to just change bacs_to_r1cs_witness_map
                        // into two :(
    ...
    const tbcs_ppzksnark_proof<ppT> &proof)
{
    ...
    const uscs_primary_input<FieldT> uscs_input =
        libff::convert_bit_vector_to_field_element_vector<FieldT>(
            primary_input);
    const tbcs_ppzksnark_processed_verification_key<ppT> pvk =
        tbcs_ppzksnark_verifier_process_vk<ppT>(vk);
    const bool bit =
        uscs_ppzksnark_online_verifier_weak_IC<ppT>(pvk, uscs_input, proof);
    ...
    const tbcs_ppzksnark_proof<ppT> &proof)
{
    ...
    const uscs_primary_input<FieldT> uscs_input =
        libff::convert_bit_vector_to_field_element_vector<FieldT>(
            primary_input);
    const bool bit =
        uscs_ppzksnark_online_verifier_strong_IC<ppT>(pvk, uscs_input, proof);
