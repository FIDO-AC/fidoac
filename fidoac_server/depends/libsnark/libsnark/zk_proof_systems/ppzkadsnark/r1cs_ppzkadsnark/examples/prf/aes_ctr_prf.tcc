...
template<> aesPrfKeyT prfGen<default_r1cs_ppzkadsnark_pp>()
{
    aesPrfKeyT key;
...
template<>
libff::Fr<snark_pp<default_r1cs_ppzkadsnark_pp>> prfCompute<
    default_r1cs_ppzkadsnark_pp>(const aesPrfKeyT &key, const labelT &label)
{
    ...
    crypto_core_aes128encrypt_openssl(
        seed_bytes, label.label_bytes, key.key_bytes, NULL);

    // use first 128 bits of output to seed AES-CTR
    // PRG to expand to 3*128 bits
    crypto_core_aes128encrypt_openssl(
        random_bytes, seed_bytes, key.key_bytes + 16, NULL);
    ...
    crypto_core_aes128encrypt_openssl(
        random_bytes + 16, seed_bytes, key.key_bytes + 16, NULL);
    ...
    crypto_core_aes128encrypt_openssl(
        random_bytes + 32, seed_bytes, key.key_bytes + 16, NULL);
    ...
    return libff::Fr<snark_pp<default_r1cs_ppzkadsnark_pp>>(
        libff::bigint<
            libff::Fr<snark_pp<default_r1cs_ppzkadsnark_pp>>::num_limbs>(aux));
}
