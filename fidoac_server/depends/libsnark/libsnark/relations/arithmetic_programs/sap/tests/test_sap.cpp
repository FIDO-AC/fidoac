...
void test_sap(
    const size_t sap_degree, const size_t num_inputs, const bool binary_input)
{
    /*
    ...
    */
    ...
    libff::print_indent();
    printf("* Requested SAP degree: %zu\n", sap_degree);
    libff::print_indent();
    printf(
        "* Actual SAP degree: %zu\n", 2 * num_constraints + 2 * num_inputs + 1);
    libff::print_indent();
    ...
    if (binary_input) {
        example = generate_r1cs_example_with_binary_input<FieldT>(
            num_constraints, num_inputs);
    } else {
        example = generate_r1cs_example_with_field_input<FieldT>(
            num_constraints, num_inputs);
    }
    ...
    const FieldT t = FieldT::random_element(), d1 = FieldT::random_element(),
                 d2 = FieldT::random_element();

    libff::enter_block("Compute SAP instance 1");
    sap_instance<FieldT> sap_inst_1 =
        r1cs_to_sap_instance_map(example.constraint_system);
    libff::leave_block("Compute SAP instance 1");

    libff::enter_block("Compute SAP instance 2");
    sap_instance_evaluation<FieldT> sap_inst_2 =
        r1cs_to_sap_instance_map_with_evaluation(example.constraint_system, t);
    libff::leave_block("Compute SAP instance 2");

    libff::enter_block("Compute SAP witness");
    sap_witness<FieldT> sap_wit = r1cs_to_sap_witness_map(
...
int main()
{
    ...
    const size_t extended_domain_size_special =
        (1ul << (libff::mnt6_Fr::s + 1)) - 1ul;

    libff::enter_block("Test SAP with binary input");

    test_sap<libff::Fr<libff::mnt6_pp>>(
        basic_domain_size_special, num_inputs, true);
    test_sap<libff::Fr<libff::mnt6_pp>>(
        step_domain_size_special, num_inputs, true);
    test_sap<libff::Fr<libff::mnt6_pp>>(
        extended_domain_size_special, num_inputs, true);
    ...
    test_sap<libff::Fr<libff::mnt6_pp>>(
        basic_domain_size_special, num_inputs, false);
    test_sap<libff::Fr<libff::mnt6_pp>>(
        step_domain_size_special, num_inputs, false);
    test_sap<libff::Fr<libff::mnt6_pp>>(
        extended_domain_size_special, num_inputs, false);
