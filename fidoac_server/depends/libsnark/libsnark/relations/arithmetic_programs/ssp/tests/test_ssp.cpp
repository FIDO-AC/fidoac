    ...
    const bool binary_input)
{
    ...
    if (binary_input) {
        example = generate_uscs_example_with_binary_input<FieldT>(
            num_constraints, num_inputs);
    } else {
        example = generate_uscs_example_with_field_input<FieldT>(
            num_constraints, num_inputs);
    }
    ...
    const FieldT t = FieldT::random_element(), d = FieldT::random_element();

    libff::enter_block("Compute SSP instance 1");
    ssp_instance<FieldT> ssp_inst_1 =
        uscs_to_ssp_instance_map(example.constraint_system);
    libff::leave_block("Compute SSP instance 1");

    libff::enter_block("Compute SSP instance 2");
    ssp_instance_evaluation<FieldT> ssp_inst_2 =
    ...
    ssp_witness<FieldT> ssp_wit = uscs_to_ssp_witness_map(
...
int main()
{
    ...
    test_ssp<libff::Fr<libff::mnt6_pp>>(
