...
void test_qap(
    const size_t qap_degree, const size_t num_inputs, const bool binary_input)
{
    /*
      We construct an instance where the QAP degree is qap_degree.
      So we generate an instance of R1CS where the number of constraints
      qap_degree - num_inputs - 1. See the transformation from R1CS to QAP for
      why this is the case. So we need that qap_degree >= num_inputs + 1.
    */
    ...
    const FieldT t = FieldT::random_element(), d1 = FieldT::random_element(),
                 d2 = FieldT::random_element(), d3 = FieldT::random_element();

    libff::enter_block("Compute QAP instance 1");
    qap_instance<FieldT> qap_inst_1 =
        r1cs_to_qap_instance_map(example.constraint_system);
    libff::leave_block("Compute QAP instance 1");

    libff::enter_block("Compute QAP instance 2");
    qap_instance_evaluation<FieldT> qap_inst_2 =
    ...
    qap_witness<FieldT> qap_wit = r1cs_to_qap_witness_map(
...
int main()
{
    ...
    test_qap<libff::Fr<libff::mnt6_pp>>(
    ...
    test_qap<libff::Fr<libff::mnt6_pp>>(
        extended_domain_size, num_inputs, false);
    test_qap<libff::Fr<libff::mnt6_pp>>(
