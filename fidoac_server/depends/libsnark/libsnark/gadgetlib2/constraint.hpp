 ...
 *****************************************************************************
 ...
 A constraint is an algebraic equation which can be either satisfied by an
 assignment, (the equation is true with that assignment) or unsatisfied. For
 instance the rank-1 constraint (X * Y = 15) is satisfied by {X=5 Y=3} or {X=3
 Y=5}
 *****************************************************************************
...
/// An abstract class for a field agnostic constraint. The derived classes will
/// be field specific.
class Constraint
{
public:
    explicit Constraint(
        const ::std::string &name); // casting disallowed by 'explicit'
    ...
    variable.
        @param[in] printOnFail - when set to true, an unsatisfied constraint
    will print to stderr information explaining why it is not satisfied.
...
/// * <b,x> = <c,x> where x is an assignment of field elements to the variables.
...
public:
    ...
    virtual const Variable::set getUsedVariables()
        const; /**< @returns a list of all variables
    ...
    **/
    ...
    PolyPtrSet getConstraintPolynomials() const
    {
        ...
        for (const auto &pConstraint : constraintsPtrs_) {
            retset.insert(::std::unique_ptr<Polynomial>(
                new Polynomial(pConstraint->asPolynomial())));
        }
        return retset;
    }
