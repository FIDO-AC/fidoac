 ...
 * follows. (1) The "generator", which runs the ppzkADSNARK generator on input a
 ...
 * (3) The "verifier", which runs the ppzkADSNARK verifier on input the
 * verification key, a primary input for CS, and a proof.
 */
template<typename ppT>
bool run_r1cs_ppzkadsnark(
    const r1cs_example<libff::Fr<snark_pp<ppT>>> &example,
    const bool test_serialization)
{
    libff::enter_block("Call to run_r1cs_ppzkadsnark");

    r1cs_ppzkadsnark_auth_keys<ppT> auth_keys =
        r1cs_ppzkadsnark_auth_generator<ppT>();

    libff::print_header("R1CS ppzkADSNARK Generator");
    r1cs_ppzkadsnark_keypair<ppT> keypair = r1cs_ppzkadsnark_generator<ppT>(
        example.constraint_system, auth_keys.pap);
    printf("\n");
    libff::print_indent();
    ...
    if (test_serialization) {
        ...
        keypair.pk =
            libff::reserialize<r1cs_ppzkadsnark_proving_key<ppT>>(keypair.pk);
        keypair.vk = libff::reserialize<r1cs_ppzkadsnark_verification_key<ppT>>(
            keypair.vk);
    ...
    bool auth_res = r1cs_ppzkadsnark_auth_verify<ppT>(
        data, auth_data, auth_keys.sak, labels);
    printf("* The verification result is: %s\n", (auth_res ? "PASS" : "FAIL"));

    libff::print_header("R1CS ppzkADSNARK Verify Public");
    bool auth_resp = r1cs_ppzkadsnark_auth_verify<ppT>(
        data, auth_data, auth_keys.pak, labels);
    ...
    bool ans = r1cs_ppzkadsnark_verifier<ppT>(
        keypair.vk, proof, auth_keys.sak, labels);
    printf("\n");
    libff::print_indent();
    ...
    bool ans2 = r1cs_ppzkadsnark_online_verifier<ppT>(
        pvk, proof, auth_keys.sak, labels);
    assert(ans == ans2);

    libff::print_header("R1CS ppzkADSNARK Public Verifier");
    ans = r1cs_ppzkadsnark_verifier<ppT>(
        keypair.vk, auth_data, proof, auth_keys.pak, labels);
    printf("\n");
    libff::print_indent();
    libff::print_mem("after verifier");
    printf("* The verification result is: %s\n", (ans ? "PASS" : "FAIL"));
    ...
    ans2 = r1cs_ppzkadsnark_online_verifier<ppT>(
        pvk, auth_data, proof, auth_keys.pak, labels);
    assert(ans == ans2);
