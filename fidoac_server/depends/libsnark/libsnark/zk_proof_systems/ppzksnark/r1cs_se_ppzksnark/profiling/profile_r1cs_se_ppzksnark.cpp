 ...
 *****************************************************************************
 ...
 The command
     ...
     $
 libsnark/zk_proof_systems/ppzksnark/r1cs_se_ppzksnark/profiling/profile_r1cs_se_ppzksnark
 1000 10 Fr

 exercises the SEppzkSNARK (first generator, then prover, then verifier) on an
...
int main(int argc, const char *argv[])
{
    ...
    if (argc == 4) {
        ...
        if (strcmp(argv[3], "bytes") == 0) {
            input_size = libff::div_ceil(
                8 * input_size, libff::Fr<libff::default_ec_pp>::capacity());
        }
    }

    libff::enter_block("Generate R1CS example");
    r1cs_example<libff::Fr<default_r1cs_se_ppzksnark_pp>> example =
        generate_r1cs_example_with_field_input<
            libff::Fr<default_r1cs_se_ppzksnark_pp>>(
            num_constraints, input_size);
    libff::leave_block("Generate R1CS example");

    libff::print_header("(enter) Profile R1CS SEppzkSNARK");
    const bool test_serialization = true;
    run_r1cs_se_ppzksnark<default_r1cs_se_ppzksnark_pp>(
