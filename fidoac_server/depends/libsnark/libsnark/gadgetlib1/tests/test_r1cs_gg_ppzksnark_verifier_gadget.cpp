    ...
    const std::string &annotation_A, const std::string &annotation_B)
{
    ...
    const r1cs_gg_ppzksnark_keypair<ppT_A> keypair =
        r1cs_gg_ppzksnark_generator<ppT_A>(example.constraint_system, true);
    const r1cs_gg_ppzksnark_proof<ppT_A> pi = r1cs_gg_ppzksnark_prover<ppT_A>(
        keypair.pk, example.primary_input, example.auxiliary_input, true);
    bool bit = r1cs_gg_ppzksnark_verifier_strong_IC<ppT_A>(
