...
/*
    This test gives the first example of a construction of a constraint system.
   We use the terms 'Constraint System' and 'Circuit' interchangeably rather
   loosely. It is easy to visualize a circuit with inputs and outputs. Each gate
   imposes some logic on the inputs and output wires. For instance, AND(inp1,
   inp2) will impose the 'constraint' (inp1 & inp2 = out) Thus, we can also
   think of this circuit as a system of constraints. Each gate is a mathematical
    constraint and each wire is a variable. In the AND example over a boolean
   field {0,1} we would write the constraint as (inp1 * inp2 == out). This
   constraint is 'satisfied' relative to an assignment if we assign values to
   {inp1, inp2, out} such that the constraint evaluates to TRUE. All following
   examples will be either field agnostic or of a specific form of prime fields:
    (1) Field agnostic case: In these examples we create high level circuits by
   using lower level circuits. This way we can ignore the specifics of a field
   and assume the lower level takes care of this. If we must explicitly write
   constraints in these circuits, they will always be very basic constraints
   which are defined over every field (e.g. x + y = 0). (2) All field specific
   examples in this library are for a prime characteristic field with the
        special form of 'quadratic rank 1 polynomials', or R1P. This is the only
   form used with the current implementation of SNARKs. The form for these
   constraints is (Linear Combination) * (Linear Combination) == (Linear
   Combination). The library has been designed to allow future addition of other
   characteristics/forms in the future by implementing only low level circuits
   for these forms.
*/
...
/*
    ...
    In this example we will construct a very simple gadget, one that implements
   a NAND gate. This gadget is field-agnostic as it only uses lower level
   gadgets and the field elements '0' & '1'.

    Gadgets are the framework which allow us to delegate construction of
   sophisticated circuitry to lower levels. Each gadget can construct a
   constraint system or a witness or both, by defining constraints and
   assignments as well as by utilizing sub-gadgets.
*/

class NAND_Gadget : public Gadget
{
public:
    // This is a convention we use to always create gadgets as if from a factory
    // class. This will be needed later for gadgets which have different
    // implementations in different fields.
    static GadgetPtr create(
        ProtoboardPtr pb,
        const FlagVariableArray &inputs,
        const FlagVariable &output);
    // generateConstraints() is the method which creates all constraints on the
    // protoboard
    void generateConstraints();
    // generateWitness() is the method which generates the witness by assigning
    // a valid value to each wire in the circuit (variable) and putting this on
    // the protoboard
    void generateWitness();

private:
    // constructor is private in order to stick to the convention that gadgets
    // are created using a create() method. This may not make sense now, but
    // when dealing with non-field agnostic gadgets it is very convenient to
    // have a factory class with this convention. Notice the protoboard. This
    // can be thought of as a 'memory manager' which holds the circuit as the
    // constraints are being built, and the 'wire values' as the witness is
    // being built
    NAND_Gadget(
        ProtoboardPtr pb,
        const FlagVariableArray &inputs,
        const FlagVariable &output);
    // init() does any non trivial work which we don't want in the constructor.
    // This is where we will 'wire' the sub-gadgets into the circuit. Each
    // sub-gadget can be thought of as a circuit gate with some specific
    // functionality.
    void init();
    // we want every gadget to be explicitly constructed
    DISALLOW_COPY_AND_ASSIGN(NAND_Gadget);

    // This is an internal gadget. Once a gadget is created it can be used as a
    // black box gate. We will initialize this pointer to be an AND_Gadget in
    // the init() method.
    GadgetPtr andGadget_;
    // These are internal variables used by the class. They will always include
    // the variables from the constructor, but can include many more as well.
    // Notice that almost always the variables can be declared 'const', as these
    // are local copies of formal variables, and do not change over the span of
    // the class' lifetime.
...
/*
    Another example showing the use of DualVariable. A DualVariable is a
   variable which holds both a bitwise representation of a word and a packed
   representation (e.g. both the packed value {42} and the unpacked value
   {1,0,1,0,1,0}). If the word is short enough (for example any integer smaller
   than the prime characteristic) then the packed representation will be stored
   in 1 field element. 'word' in this context means a set of bits, it is a
    convention which means we expect some semantic ability to decompose the
   packed value into its bits. The use of DualVariables is for efficiency
   reasons. More on this at the end of this example. In this example we will
   construct a gadget which receives as input a packed integer value called
   'hash', and a 'difficulty' level in bits, and constructs a circuit validating
   that the first 'difficulty' bits of 'hash' are '0'. For simplicity we will
   assume 'hash' is always 64 bits long.
*/
...
private:
    ...
    DualWord hashValue_;
    // This GadgetPtr will be a gadget to unpack hashValue_ from packed
    // representation to bit representation. Recall 'DualWord' holds both
    // values, but only the packed version will be received as input to the
    // constructor.
...
// IMPLEMENTATION
HashDifficultyEnforcer_Gadget::HashDifficultyEnforcer_Gadget(
    ProtoboardPtr pb,
    const MultiPackedWord &hashValue,
    const size_t difficultyBits)
    : Gadget(pb)
...
{
    // because we are using a prime field with large characteristic, we can
    // assume a 64 bit value fits in the first element of a multipacked
    // variable.
    GADGETLIB_ASSERT(
        hashValue_.multipacked().size() == 1,
        "multipacked word size too large");
    // A DualWord_Gadget's constraints assert that the unpacked and packed
    // values represent the same integer element. The generateWitness() method
    // has two modes, one for packing (taking the bit representation as input)
    // and one for unpacking (creating the bit representation from the packed
    // representation)
    hashValueUnpacker_ =
        DualWord_Gadget::create(pb_, hashValue_, PackingMode::UNPACK);
}

GadgetPtr HashDifficultyEnforcer_Gadget::create(
    ProtoboardPtr pb,
    const MultiPackedWord &hashValue,
    const size_t difficultyBits)
{
    GadgetPtr pGadget(
        new HashDifficultyEnforcer_Gadget(pb, hashValue, difficultyBits));
    pGadget->init();
    return pGadget;
}
...
void HashDifficultyEnforcer_Gadget::generateConstraints()
{
    ...
    // add constraints asserting that the first 'difficultyBits' bits of
    // 'hashValue' equal 0. Note endianness, unpacked()[0] is LSB and
    // unpacked()[63] is MSB
    for (size_t i = 0; i < difficultyBits_; ++i) {
        addUnaryConstraint(
            hashValue_.unpacked()[63 - i],
            GADGETLIB2_FMT("hashValue[%u] == 0", 63 - i));
    }
}
...
void HashDifficultyEnforcer_Gadget::generateWitness()
{
    ...
    // In a real setting we would add an assertion that the value will indeed
    // satisfy the difficulty constraint, and notify the user with an error
    // otherwise. As this is a tutorial, we'll let invalid values pass through
    // so that we can see how isSatisfied() returns false.
}

// Remember we pointed out that DualVariables are used for efficiency reasons.
// Now is the time to elaborate on this. As you've seen, we needed a bit
// representation in order to check the first bits of hashValue. But hashValue
// may be used in many other places, for instance we may want to check equality
// with another value. Checking equality on a packed representation will 'cost'
// us 1 constraint, while checking equality on the unpacked value will 'cost' us
// 64 constraints. This translates heavily into proof construction time and
// memory in the ppzkSNARK proof system.
...
/*
    In this example we will construct a gadget which builds a circuit for proof
   (witness) and validation (constraints) that a bitcoin transaction's sum of
   inputs equals the sum of outputs + miners fee. Construction of the proof will
   include finding the miners' fee. This fee can be thought of as an output of
   the circuit.

    This is a field specific gadget, as we will use the '+' operator freely. The
   addition operation works as expected over integers while in prime
   characteristic fields but not so in extension fields. If you are not familiar
   with extension fields, don't worry about it. Simply be aware that + and *
   behave differently in different fields and don't necessarily give the integer
   values you would expect.

    The library design supports multiple field constructs due to different
   applied use cases. Some cryptographic applications may need extension fields
   while others may need prime fields, but with constraints which are not
   rank-1, and yet others may need boolean circuits. The library was designed so
   that high level gadgets can be reused by implementing only the low level for
    a new field or constraint structure.

    Later we will supply a recipe for creation of such field specific gadgets
   with agnostic interfaces. We use a few conventions here in order to ease the
   process by using macros.
*/

// This is a macro which creates an interface class for all field specific
// derived gadgets Convention is: class {GadgetName}_GadgetBase
CREATE_GADGET_BASE_CLASS(VerifyTransactionAmounts_GadgetBase);

// Notice the multiple inheritance. We must specify the interface as well as the
// field specific base gadget. This is what allows the factory class to decide
// at compile time which field specific class to instantiate for every
// protoboard. See design notes in "gadget.hpp" Convention is: class
// {FieldType}_{GadgetName}_Gadget
...
public:
    ...
    // We give the factory class friend access in order to instantiate via
    // private constructor.
...
// create factory class using CREATE_GADGET_FACTORY_CLASS_XX macro (substitute
// XX with the number of arguments to the constructor, excluding the
// protoboard). Sometimes we want multiple constructors, see AND_Gadget for
// example. In this case we will have to manually write the factory class.
...
/*
    As promised, recipe for creating field specific gadgets with agnostic
   interfaces:
    ...
    (3) Create any field specific gadgets with multiple inheritance:
        ...
        Notice all arguments to the constructors must be const& in order to use
   the factory class macro. Constructor arguments must be the same for all field
   specific implementations. (4) Give the factory class {GadgetName}_Gadget
   public friend access to the field specific classes. (5) Create the factory
   class using the macro: CREATE_GADGET_FACTORY_CLASS_XX({GadgetName}_Gadget,
   type1, input1, type2, input2, ... , typeXX, inputXX);
*/
...
/*
    Below is an example of integrating gadgetlib2 constructed constraint systems
   with the ppzkSNARK.
*/
