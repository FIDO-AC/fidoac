 ...
 *****************************************************************************
 ...
 Functions to profile the gadgetlib1 implementations of Benes and AS-Waksman
 routing networks.
    ...
    size_t &num_variables)
{
    ...
    as_waksman_routing_gadget<FieldT> r(
        pb, n, randbits, outbits, "main_routing_gadget");
    ...
    size_t &num_variables)
{
    ...
    std::vector<pb_variable_array<FieldT>> randbits(1ul << t),
        outbits(1ul << t);
    ...
    benes_routing_gadget<FieldT> r(
        pb, n, randbits, outbits, n, "main_routing_gadget");
    r.generate_r1cs_constraints();
...
template<typename FieldT> void profile_routing_gadgets(const size_t l)
{
    ...
    for (size_t n = 2; n <= 65; ++n) {
        ...
        printf(
            "n = %zu (rounded = %zu), l = %zu, benes_constr = %zu, benes_vars "
            "= %zu, as_waksman_constr = %zu, as_waksman_vars = %zu, "
            "constr_ratio = %0.3f, var_ratio = %0.3f\n",
            n,
            rounded_n,
            l,
            benes_constr,
            ...
            as_waksman_constr,
            as_waksman_vars,
            1. * benes_constr / as_waksman_constr,
            1. * benes_vars / as_waksman_vars);
    }
}

template<typename FieldT> void profile_num_switches(const size_t l)
{
    printf("profiling number of switches in arbitrary size networks (and "
           "rounded-up for Benes)\n");
    for (size_t n = 2; n <= 65; ++n) {
        size_t as_waksman_constr, as_waksman_vars;
        get_as_waksman_size<FieldT>(n, l, as_waksman_constr, as_waksman_vars);
        ...
        const size_t as_waksman_switches =
            (as_waksman_constr - n * (2 + l)) / 2;
        const size_t benes_switches = (benes_constr - rounded_n * (2 + l)) / 2;
        // const size_t benes_expected = libff::log2(rounded_n)*rounded_n; //
        // switch-Benes has (-rounded_n/2) term
        printf(
            "n = %zu (rounded_n = %zu), l = %zu, benes_switches = %zu, "
            "as_waksman_switches = %zu, ratio = %0.3f\n",
            n,
            rounded_n,
            l,
            benes_switches,
            as_waksman_switches,
            1. * benes_switches / as_waksman_switches);
    }
}
