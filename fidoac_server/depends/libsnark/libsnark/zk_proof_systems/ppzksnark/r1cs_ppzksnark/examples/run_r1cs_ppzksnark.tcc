...
typename std::enable_if<ppT::has_affine_pairing, void>::type test_affine_verifier(
    ...
    const bool expected_answer)
{
    ...
    const bool answer =
        r1cs_ppzksnark_affine_verifier_weak_IC<ppT>(vk, primary_input, proof);
    ...
    const r1cs_example<libff::Fr<ppT>> &example, const bool test_serialization)
{
    ...
    if (test_serialization) {
        ...
        keypair.pk =
            libff::reserialize<r1cs_ppzksnark_proving_key<ppT>>(keypair.pk);
        keypair.vk = libff::reserialize<r1cs_ppzksnark_verification_key<ppT>>(
            keypair.vk);
        pvk =
            libff::reserialize<r1cs_ppzksnark_processed_verification_key<ppT>>(
                pvk);
    ...
    const bool ans = r1cs_ppzksnark_verifier_strong_IC<ppT>(
    ...
    const bool ans2 = r1cs_ppzksnark_online_verifier_strong_IC<ppT>(
