...
tally_pcd_message<FieldT>::tally_pcd_message(
    const size_t type,
...
template<typename FieldT>
r1cs_variable_assignment<FieldT> tally_pcd_message<
    FieldT>::payload_as_r1cs_variable_assignment() const
{
    std::function<FieldT(bool)> bit_to_FieldT = [](const bool bit) {
        return bit ? FieldT::one() : FieldT::zero();
    };

    const libff::bit_vector sum_bits =
        libff::convert_field_element_to_bit_vector<FieldT>(sum, wordsize);
    const libff::bit_vector count_bits =
        libff::convert_field_element_to_bit_vector<FieldT>(count, wordsize);

    r1cs_variable_assignment<FieldT> result(2 * wordsize);
    std::transform(
        sum_bits.begin(), sum_bits.end(), result.begin(), bit_to_FieldT);
    std::transform(
        count_bits.begin(),
        count_bits.end(),
...
template<typename FieldT> void tally_pcd_message<FieldT>::print() const
{
...
template<typename FieldT>
r1cs_variable_assignment<FieldT> tally_pcd_local_data<
...
template<typename FieldT> void tally_pcd_local_data<FieldT>::print() const
{
...
public:
        ...
        , wordsize(wordsize)
    {
        ...
        count_bits.allocate(
            pb, wordsize, FMT(annotation_prefix, " count_bits"));
    ...
    std::shared_ptr<r1cs_pcd_message<FieldT>> get_message() const
    {
        ...
        const size_t sum_val =
            sum_bits.get_field_element_from_bits(this->pb).as_ulong();
        const size_t count_val =
            count_bits.get_field_element_from_bits(this->pb).as_ulong();

        std::shared_ptr<r1cs_pcd_message<FieldT>> result;
        result.reset(new tally_pcd_message<FieldT>(
...
template<typename FieldT>
class tally_pcd_local_data_variable
    : public r1cs_pcd_local_data_variable<FieldT>
{
public:
    pb_variable<FieldT> summand;

    tally_pcd_local_data_variable(
...
tally_cp_handler<FieldT>::tally_cp_handler(
    ...
    , wordsize(wordsize)
{
    this->outgoing_message.reset(new tally_pcd_message_variable<FieldT>(
        this->pb, wordsize, "outgoing_message"));
    this->arity.allocate(this->pb, "arity");

    for (size_t i = 0; i < max_arity; ++i) {
        this->incoming_messages[i].reset(new tally_pcd_message_variable<FieldT>(
            this->pb, wordsize, FMT("", "incoming_messages_%zu", i)));
    }

    this->local_data.reset(
        new tally_pcd_local_data_variable<FieldT>(this->pb, "local_data"));
    ...
    compute_type_val_inner_product.reset(new inner_product_gadget<FieldT>(
        this->pb,
        incoming_types,
        sum_in_packed,
    ...
    unpack_sum_out.reset(new packing_gadget<FieldT>(
        this->pb,
        std::dynamic_pointer_cast<tally_pcd_message_variable<FieldT>>(
            this->outgoing_message)
            ->sum_bits,
        sum_out_packed,
        "pack_sum_out"));
    unpack_count_out.reset(new packing_gadget<FieldT>(
        this->pb,
        std::dynamic_pointer_cast<tally_pcd_message_variable<FieldT>>(
            this->outgoing_message)
            ->count_bits,
    ...
    for (size_t i = 0; i < max_arity; ++i) {
        pack_sum_in.emplace_back(packing_gadget<FieldT>(
            this->pb,
            std::dynamic_pointer_cast<tally_pcd_message_variable<FieldT>>(
                this->incoming_messages[i])
                ->sum_bits,
            sum_in_packed[i],
            FMT("", "pack_sum_in_%zu", i)));
        pack_count_in.emplace_back(packing_gadget<FieldT>(
            this->pb,
            std::dynamic_pointer_cast<tally_pcd_message_variable<FieldT>>(
                this->incoming_messages[i])
                ->sum_bits,
            count_in_packed[i],
            FMT("", "pack_count_in_%zu", i)));
    }
...
void tally_cp_handler<FieldT>::generate_r1cs_constraints()
{
    ...
    for (size_t i = 0; i < this->max_arity; ++i) {
        this->pb.add_r1cs_constraint(
            r1cs_constraint<FieldT>(
                incoming_types[i], sum_in_packed_aux[i], sum_in_packed[i]),
            FMT("", "initial_sum_%zu_is_zero", i));
        this->pb.add_r1cs_constraint(
            r1cs_constraint<FieldT>(
                incoming_types[i], count_in_packed_aux[i], count_in_packed[i]),
            FMT("", "initial_sum_%zu_is_zero", i));
    }

    /* constrain arity indicator variables so that arity_indicators[arity] = 1
     * and arity_indicators[i] = 0 for any other i */
    for (size_t i = 0; i < this->max_arity; ++i) {
        this->pb.add_r1cs_constraint(
            r1cs_constraint<FieldT>(
                this->arity - FieldT(i), arity_indicators[i], 0),
            FMT("", "arity_indicators_%zu", i));
    }
    ...
    /* require that types of messages that are past arity (i.e. unbound wires)
     * carry 0 */
    for (size_t i = 0; i < this->max_arity; ++i) {
        this->pb.add_r1cs_constraint(
            r1cs_constraint<FieldT>(
        ...
        r1cs_constraint<FieldT>(
            ...
            type_val_inner_product +
                std::dynamic_pointer_cast<
                    tally_pcd_local_data_variable<FieldT>>(this->local_data)
                    ->summand,
            sum_out_packed),
        "update_sum");
    ...
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(
            1, 1 + pb_sum<FieldT>(count_in_packed), count_out_packed),
        "update_count");
}

template<typename FieldT>
void tally_cp_handler<FieldT>::generate_r1cs_witness(
    ...
    const std::shared_ptr<r1cs_pcd_local_data<FieldT>> &local_data)
{
    ...
    for (size_t i = 0; i < this->max_arity; ++i) {
        ...
        if (!this->pb.val(incoming_types[i]).is_zero()) {
            this->pb.val(sum_in_packed_aux[i]) =
                this->pb.val(sum_in_packed[i]) *
                this->pb.val(incoming_types[i]).inverse();
            this->pb.val(count_in_packed_aux[i]) =
                this->pb.val(count_in_packed[i]) *
                this->pb.val(incoming_types[i]).inverse();
        }
    }
    ...
    for (size_t i = 0; i < this->max_arity + 1; ++i) {
        this->pb.val(arity_indicators[i]) =
            (incoming_messages.size() == i ? FieldT::one() : FieldT::zero());
    }

    compute_type_val_inner_product->generate_r1cs_witness();
    this->pb.val(sum_out_packed) =
        this->pb.val(
            std::dynamic_pointer_cast<tally_pcd_local_data_variable<FieldT>>(
                this->local_data)
                ->summand) +
...
template<typename FieldT>
std::shared_ptr<r1cs_pcd_message<FieldT>> tally_cp_handler<
    FieldT>::get_base_case_message() const
{
    const size_t type = 0;
    const size_t sum = 0;
