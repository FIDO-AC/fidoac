 ...
 * (3) The "verifier", which runs the ppzkSNARK verifier on input the
 * verification key, a primary input for CS, and a proof.
 */
template<typename ppT>
bool run_uscs_ppzksnark(
    const uscs_example<libff::Fr<ppT>> &example, const bool test_serialization)
{
    libff::enter_block("Call to run_uscs_ppzksnark");

    libff::print_header("USCS ppzkSNARK Generator");
    uscs_ppzksnark_keypair<ppT> keypair =
        uscs_ppzksnark_generator<ppT>(example.constraint_system);
    printf("\n");
    libff::print_indent();
    ...
    if (test_serialization) {
        ...
        keypair.pk =
            libff::reserialize<uscs_ppzksnark_proving_key<ppT>>(keypair.pk);
        keypair.vk = libff::reserialize<uscs_ppzksnark_verification_key<ppT>>(
            keypair.vk);
        pvk =
            libff::reserialize<uscs_ppzksnark_processed_verification_key<ppT>>(
                pvk);
        libff::leave_block("Test serialization of keys");
    }

    libff::print_header("USCS ppzkSNARK Prover");
    uscs_ppzksnark_proof<ppT> proof = uscs_ppzksnark_prover<ppT>(
    ...
    bool ans = uscs_ppzksnark_verifier_strong_IC<ppT>(
