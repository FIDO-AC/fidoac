    ...
    const bool test_serialization)
{
    ...
    r1cs_pcd_compliance_predicate<FieldT> tally_cp =
        tally.get_compliance_predicate();
    libff::leave_block("Generate compliance predicate");

    libff::print_header("R1CS ppzkPCD Generator");
    r1cs_sp_ppzkpcd_keypair<PCD_ppT> keypair =
        r1cs_sp_ppzkpcd_generator<PCD_ppT>(tally_cp);
    ...
    if (test_serialization) {
        ...
        keypair.pk = libff::reserialize<r1cs_sp_ppzkpcd_proving_key<PCD_ppT>>(
            keypair.pk);
        keypair.vk =
            libff::reserialize<r1cs_sp_ppzkpcd_verification_key<PCD_ppT>>(
                keypair.vk);
    ...
    for (long layer = depth; layer >= 0; --layer, nodes_in_layer /= arity) {
        for (size_t i = 0; i < nodes_in_layer; ++i) {
            ...
            if (test_serialization) {
                ...
                proof =
                    libff::reserialize<r1cs_sp_ppzkpcd_proof<PCD_ppT>>(proof);
            ...
            const r1cs_sp_ppzkpcd_primary_input<PCD_ppT> pcd_verifier_input(
                tree_messages[cur_idx]);
            const bool ans = r1cs_sp_ppzkpcd_verifier<PCD_ppT>(
                keypair.vk, pcd_verifier_input, tree_proofs[cur_idx]);

            libff::print_header("R1CS ppzkPCD Online Verifier");
            const bool ans2 = r1cs_sp_ppzkpcd_online_verifier<PCD_ppT>(
