...
set_commitment_gadget<FieldT, HashT>::set_commitment_gadget(
...
{
    element_block.reset(new block_variable<FieldT>(
        pb, {element_bits}, FMT(annotation_prefix, " element_block")));

    if (tree_depth == 0) {
        hash_element.reset(new HashT(
            pb,
            element_bits.size(),
            *element_block,
            root_digest,
            FMT(annotation_prefix, " hash_element")));
    } else {
        element_digest.reset(new digest_variable<FieldT>(
            pb,
...
template<typename FieldT, typename HashT> void test_set_commitment_gadget()
{
    const size_t digest_len = HashT::get_digest_len();
    const size_t max_set_size = 16;
    const size_t value_size =
        (HashT::get_block_len() > 0 ? HashT::get_block_len() : 10);
    ...
    for (size_t i = 0; i < max_set_size; ++i) {
        ...
        std::generate(
            elem.begin(), elem.end(), [&]() { return std::rand() % 2; });
    ...
    set_membership_proof_variable<FieldT, HashT> proof(
        pb, max_set_size, "proof");

    set_commitment_gadget<FieldT, HashT> sc(
        pb,
        max_set_size,
        element_bits,
        root_digest,
    ...
    for (size_t i = 0; i < max_set_size; ++i) {
        ...
        proof.generate_r1cs_witness(
            accumulator.get_membership_proof(set_elems[i]));
