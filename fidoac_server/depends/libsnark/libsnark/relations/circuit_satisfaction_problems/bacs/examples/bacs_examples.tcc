...
linear_combination<FieldT> random_linear_combination(const size_t num_variables)
{
    ...
    for (size_t i = 0; i < terms; ++i) {
        const FieldT coeff =
            FieldT(std::rand()); // TODO: replace with FieldT::random_element(),
                                 // when it becomes faster...
        result = result +
                 coeff * variable<FieldT>(std::rand() % (num_variables + 1));
    }
    ...
    for (size_t i = 0; i < num_gates; ++i) {
        ...
        if (i >= num_gates - num_outputs) {
            ...
            const var_index_t var_idx =
                std::rand() %
                (1 + primary_input_size + std::min(num_gates - num_outputs, i));
            const FieldT var_val =
                (var_idx == 0 ? FieldT::one() : all_vals[var_idx - 1]);
