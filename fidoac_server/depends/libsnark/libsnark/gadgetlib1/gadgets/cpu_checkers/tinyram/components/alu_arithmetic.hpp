...
public:
    ...
    {
        res_word.allocate(
            pb, pb.ap.w, FMT(this->annotation_prefix, " res_bit"));
        not_all_zeros_result.allocate(
            pb, FMT(this->annotation_prefix, " not_all_zeros_result"));
        ...
        not_all_zeros.reset(new disjunction_gadget<FieldT>(
            pb,
            res_word,
...
public:
              ...
              annotation_prefix)
    {
        addition_result.allocate(
            pb, FMT(this->annotation_prefix, " addition_result"));
        res_word.allocate(
            pb, pb.ap.w, FMT(this->annotation_prefix, " res_word"));

        res_word_and_flag = res_word;
        res_word_and_flag.emplace_back(result_flag);

        unpack_addition.reset(new packing_gadget<FieldT>(
            pb,
            res_word_and_flag,
            addition_result,
...
public:
              ...
              annotation_prefix)
    {
        intermediate_result.allocate(
            pb, FMT(this->annotation_prefix, " intermediate_result"));
        negated_flag.allocate(
            pb, FMT(this->annotation_prefix, " negated_flag"));
        res_word.allocate(
            pb, pb.ap.w, FMT(this->annotation_prefix, " res_word"));
        ...
        unpack_intermediate.reset(new packing_gadget<FieldT>(
            pb,
...
public:
    ...
    ALU_cmp_gadget(
        ...
        , cmpe_result(cmpe_result)
        , cmpe_result_flag(cmpe_result_flag)
        , cmpa_result(cmpa_result)
        , cmpa_result_flag(cmpa_result_flag)
        , cmpae_result(cmpae_result)
...
public:
    ...
    ALU_cmps_gadget(
        ...
        : ALU_arithmetic_gadget<FieldT>(
              ...
              cmpg_result_flag,
              annotation_prefix)
        , cmpg_result(cmpg_result)
        , cmpg_result_flag(cmpg_result_flag)
        , cmpge_result(cmpge_result)
        , cmpge_result_flag(cmpge_result_flag)
    {
        negated_arg1val_sign.allocate(
            pb, FMT(this->annotation_prefix, " negated_arg1val_sign"));
        negated_arg2val_sign.allocate(
            pb, FMT(this->annotation_prefix, " negated_arg2val_sign"));

        modified_arg1 = pb_variable_array<FieldT>(
            arg1val.bits.begin(), --arg1val.bits.end());
        ...
        packed_modified_arg1.allocate(
            pb, FMT(this->annotation_prefix, " packed_modified_arg1"));
        packed_modified_arg2.allocate(
            pb, FMT(this->annotation_prefix, " packed_modified_arg2"));

        pack_modified_arg1.reset(new packing_gadget<FieldT>(
            pb,
            modified_arg1,
            packed_modified_arg1,
            FMT(this->annotation_prefix, " pack_modified_arg1")));
        pack_modified_arg2.reset(new packing_gadget<FieldT>(
            pb,
            modified_arg2,
            packed_modified_arg2,
            FMT(this->annotation_prefix, " pack_modified_arg2")));

        comparator.reset(new comparison_gadget<FieldT>(
            pb,
            pb.ap.w,
            packed_modified_arg2,
            packed_modified_arg1,
...
public:
    ...
    ALU_umul_gadget(
        ...
        , mull_result(mull_result)
        , mull_flag(mull_flag)
        , umulh_result(umulh_result)
        , umulh_flag(umulh_flag)
    {
        mull_bits.insert(
            mull_bits.end(),
        ...
        pack_mull_result.reset(new packing_gadget<FieldT>(
            pb,
            mull_bits,
            mull_result,
            FMT(this->annotation_prefix, " pack_mull_result")));
        pack_umulh_result.reset(new packing_gadget<FieldT>(
            pb,
            umulh_bits,
            umulh_result,
            FMT(this->annotation_prefix, " pack_umulh_result")));

        result_flag.allocate(pb, FMT(this->annotation_prefix, " result_flag"));
        compute_flag.reset(new disjunction_gadget<FieldT>(
            pb,
            umulh_bits,
            result_flag,
            FMT(this->annotation_prefix, " compute_flag")));
    }
    void generate_r1cs_constraints();
    void generate_r1cs_witness();
};
...
public:
    ...
    ALU_smul_gadget(
        ...
        smulh_result(smulh_result)
        , smulh_flag(smulh_flag)
    {
        smulh_bits.insert(
            smulh_bits.end(),
            mul_result.bits.begin() + pb.ap.w,
        ...
        pack_smulh_result.reset(new packing_gadget<FieldT>(
            pb,
            smulh_bits,
            smulh_result,
            FMT(this->annotation_prefix, " pack_smulh_result")));

        top.allocate(pb, FMT(this->annotation_prefix, " top"));
        pack_top.reset(new packing_gadget<FieldT>(
            pb,
        ...
        is_top_empty.allocate(
            pb, FMT(this->annotation_prefix, " is_top_empty"));
        is_top_empty_aux.allocate(
            pb, FMT(this->annotation_prefix, " is_top_empty_aux"));

        is_top_full.allocate(pb, FMT(this->annotation_prefix, " is_top_full"));
        is_top_full_aux.allocate(
            pb, FMT(this->annotation_prefix, " is_top_full_aux"));
...
public:
    ...
    ALU_divmod_gadget(
        ...
        : ALU_arithmetic_gadget<FieldT>(
              ...
              udiv_flag,
              annotation_prefix)
        , udiv_result(udiv_result)
        , udiv_flag(udiv_flag)
        , umod_result(umod_result)
        , umod_flag(umod_flag)
    {
        ...
        r_less_B.reset(new comparison_gadget<FieldT>(
            pb,
            pb.ap.w,
            umod_result,
            arg2val.packed,
            B_nonzero,
            ONE,
            FMT(this->annotation_prefix, " r_less_B")));
    }
    void generate_r1cs_constraints();
    void generate_r1cs_witness();
};
...
public:
        ...
        , shl_flag(shl_flag)
    {
        ...
        reversed_input.allocate(
            pb, FMT(this->annotation_prefix, " reversed_input"));
        pack_reversed_input.reset(new packing_gadget<FieldT>(
            pb,
            pb_variable_array<FieldT>(
                arg1val.bits.rbegin(), arg1val.bits.rend()),
            reversed_input,
        ...
        barrel_right_internal.allocate(
            pb,
            logw + 1,
            FMT(this->annotation_prefix, " barrel_right_internal"));
        ...
        for (size_t i = 0; i < logw; ++i) {
            shifted_out_bits[i].allocate(
                pb,
                1ul << i,
                FMT(this->annotation_prefix, " shifted_out_bits_%zu", i));
        }

        is_oversize_shift.allocate(
            pb, FMT(this->annotation_prefix, " is_oversize_shift"));
        check_oversize_shift.reset(new disjunction_gadget<FieldT>(
            pb,
            pb_variable_array<FieldT>(
                arg2val.bits.begin() + logw, arg2val.bits.end()),
            is_oversize_shift,
            FMT(this->annotation_prefix, " check_oversize_shift")));
        result.allocate(pb, FMT(this->annotation_prefix, " result"));

        result_bits.allocate(
            pb, pb.ap.w, FMT(this->annotation_prefix, " result_bits"));
        unpack_result.reset(new packing_gadget<FieldT>(
            pb,
            result_bits,
            result, // barrel_right_internal[logw],
            FMT(this->annotation_prefix, " unpack_result")));

        reversed_result.allocate(
            pb, FMT(this->annotation_prefix, " reversed_result"));
        pack_reversed_result.reset(new packing_gadget<FieldT>(
            pb,
