...
int main(int argc, const char *argv[])
{
    ...
    if (argc != 6) {
        printf(
            "usage: %s word_size reg_count program_size input_size "
            "time_bound\n",
            argv[0]);
        return 1;
    }

    const size_t w = atoi(argv[1]), k = atoi(argv[2]),
                 program_size = atoi(argv[3]), input_size = atoi(argv[4]),
    ...
    ram_example<machine_ppT> example = gen_ram_example_complex<machine_ppT>(
        ap, boot_trace_size_bound, time_bound, satisfiable);
