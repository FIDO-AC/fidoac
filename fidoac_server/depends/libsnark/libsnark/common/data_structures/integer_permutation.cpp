...
integer_permutation::integer_permutation(
    const size_t min_element, const size_t max_element)
...
{
    return (
        this->min_element == other.min_element &&
...
bool integer_permutation::is_valid() const
{
    ...
    for (auto &el : contents) {
        if (el < min_element || el > max_element ||
            elems.find(el) != elems.end()) {
            return false;
        }
    ...
    const size_t slice_min_element, const size_t slice_max_element) const
{
    ...
    std::copy(
        this->contents.begin() + (slice_min_element - min_element),
