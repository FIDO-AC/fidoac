    ...
    const bacs_example<libff::Fr<ppT>> &example, const bool test_serialization)
{
    ...
    bacs_ppzksnark_keypair<ppT> keypair =
        bacs_ppzksnark_generator<ppT>(example.circuit);
    printf("\n");
    libff::print_indent();
    ...
    if (test_serialization) {
        ...
        keypair.pk =
            libff::reserialize<bacs_ppzksnark_proving_key<ppT>>(keypair.pk);
        keypair.vk = libff::reserialize<bacs_ppzksnark_verification_key<ppT>>(
            keypair.vk);
        pvk =
            libff::reserialize<bacs_ppzksnark_processed_verification_key<ppT>>(
                pvk);
        libff::leave_block("Test serialization of keys");
    }

    libff::print_header("BACS ppzkSNARK Prover");
    bacs_ppzksnark_proof<ppT> proof = bacs_ppzksnark_prover<ppT>(
    ...
    bool ans = bacs_ppzksnark_verifier_strong_IC<ppT>(
    ...
    bool ans2 = bacs_ppzksnark_online_verifier_strong_IC<ppT>(
