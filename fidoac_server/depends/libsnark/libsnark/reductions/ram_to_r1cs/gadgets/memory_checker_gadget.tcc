...
memory_checker_gadget<ramT>::memory_checker_gadget(
    ...
    : ram_gadget_base<ramT>(pb, annotation_prefix), line1(line1), line2(line2)
{
    ...
    timestamps_leq.allocate(
        pb, FMT(this->annotation_prefix, " timestamps_leq"));
    timestamps_less.allocate(
        pb, FMT(this->annotation_prefix, " timestamps_less"));
    compare_timestamps.reset(new comparison_gadget<FieldT>(
        pb,
        timestamp_size,
    ...
    addresses_less.allocate(
        pb, FMT(this->annotation_prefix, " addresses_less"));
    compare_addresses.reset(new comparison_gadget<FieldT>(
        pb,
        address_size,
    ...
    /*
      ...
      relations:
      - "line1.contents_after = line2.contents_before" (to check that contents
      do not change between instructions);
      - "line2.contents_before = 0" (for the first access at an address); and
      - "line2.timestamp = 0" (for wrap-around checks to ensure only one 'cycle'
      in the memory sort).

      More precisely, each of the above flags is "loose" (i.e., it equals 0 if
      the relation holds, but can be either 0 or 1 if the relation does not
      hold).
     */
    ...
    loose_timestamp2_is_zero.allocate(
        pb, FMT(this->annotation_prefix, " loose_timestamp2_is_zero"));
}
...
void memory_checker_gadget<ramT>::generate_r1cs_constraints()
{
    ...
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(
            addresses_leq, 1 - addresses_less, addresses_eq),
        FMT(this->annotation_prefix, " addresses_eq"));
    ...
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(
            loose_timestamp2_is_zero, line2.timestamp->packed, 0),
    ...
    /*
      ...
      line1.address = line2.address => line1.contents_after =
      line2.contents_before (i.e. contents do not change between accesses to the
      same address)
    ...
    */
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(
            addresses_eq, 1 - loose_contents_after1_equals_contents_before2, 0),
        FMT(this->annotation_prefix,
            " memory_retains_contents_between_accesses"));
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(
            addresses_less, 1 - loose_contents_before2_equals_zero, 0),
        FMT(this->annotation_prefix, " new_address_starts_at_zero"));
    this->pb.add_r1cs_constraint(
        r1cs_constraint<FieldT>(
            1 - addresses_leq, 1 - loose_timestamp2_is_zero, 0),
...
void memory_checker_gadget<ramT>::generate_r1cs_witness()
{
    ...
    this->pb.val(addresses_eq) = this->pb.val(addresses_leq) *
                                 (FieldT::one() - this->pb.val(addresses_less));
    ...
    this->pb.val(loose_contents_after1_equals_contents_before2) =
        (this->pb.val(line1.contents_after->packed) ==
         this->pb.val(line2.contents_before->packed))
            ? FieldT::one()
            : FieldT::zero();
    this->pb.val(loose_contents_before2_equals_zero) =
        this->pb.val(line2.contents_before->packed).is_zero() ? FieldT::one()
                                                              : FieldT::zero();
    this->pb.val(loose_timestamp2_is_zero) =
        (this->pb.val(line2.timestamp->packed) == FieldT::zero()
             ? FieldT::one()
             : FieldT::zero());
}
