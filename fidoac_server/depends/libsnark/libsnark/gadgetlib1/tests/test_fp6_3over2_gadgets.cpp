...
TEST(Fp6_3over2_Test, MulGadgetTest)
{
    ...
    protoboard<FieldT> pb;
    Fp6_3over2_variable<Fp6T> a_var(pb, "a");
    Fp6_3over2_variable<Fp6T> b_var(pb, "b");
    Fp6_3over2_variable<Fp6T> c_var(pb, "c");
    const size_t num_primary_inputs = pb.num_inputs();
    pb.set_input_sizes(num_primary_inputs);

    Fp6_3over2_mul_gadget<Fp6T> mul_a_b(pb, a_var, b_var, c_var, "a*b");
