...
digest_variable<FieldT>::digest_variable(
...
void digest_variable<FieldT>::generate_r1cs_constraints()
{
    for (size_t i = 0; i < digest_size; ++i) {
        generate_boolean_r1cs_constraint<FieldT>(
            this->pb, bits[i], FMT(this->annotation_prefix, " bits_%zu", i));
    }
}

template<typename FieldT>
void digest_variable<FieldT>::generate_r1cs_witness(
    const libff::bit_vector &contents)
{
...
template<typename FieldT>
block_variable<FieldT>::block_variable(
    protoboard<FieldT> &pb,
...
template<typename FieldT>
void block_variable<FieldT>::generate_r1cs_witness(
    const libff::bit_vector &contents)
{
    bits.fill_with_bits(this->pb, contents);
}
