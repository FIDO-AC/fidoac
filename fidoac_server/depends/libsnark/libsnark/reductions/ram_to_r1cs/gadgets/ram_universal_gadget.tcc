...
ram_universal_gadget<ramT>::ram_universal_gadget(
...
{
    num_memory_lines =
        boot_trace_size_bound + (time_bound + 1) +
        time_bound; /* boot lines, (time_bound + 1) execution lines (including
                       initial) and time_bound load instruction lines */
    ...
    execution_lines.emplace_back(execution_line_variable_gadget<ramT>(
        pb,
        timestamp_size,
        pb.ap,
        FMT(annotation_prefix, " execution_lines_%zu", 0)));
    ...
    for (size_t i = 0; i < boot_trace_size_bound; ++i) {
        boot_lines.emplace_back(memory_line_variable_gadget<ramT>(
            pb,
            timestamp_size,
            pb.ap,
            FMT(annotation_prefix, " boot_lines_%zu", i)));
    ...
    load_instruction_lines.reserve(
        time_bound +
    ...
    for (size_t i = 0; i < time_bound; ++i) {
        load_instruction_lines.emplace_back(memory_line_variable_gadget<ramT>(
            pb,
            timestamp_size,
            pb.ap,
            FMT(annotation_prefix, " load_instruction_lines_%zu", i)));
        ...
        execution_lines.emplace_back(execution_line_variable_gadget<ramT>(
            ...
            timestamp_size,
            pb.ap,
            FMT(annotation_prefix, " execution_lines_%zu", i + 1)));
    ...
    load_instruction_lines.emplace_back(memory_line_variable_gadget<ramT>(
        ...
        timestamp_size,
        pb.ap,
        FMT(annotation_prefix, " load_instruction_lines_%zu", time_bound)));
    ...
    const size_t packed_line_size =
        libff::div_ceil(line_size_bits, max_chunk_size);
    ...
    for (size_t i = 0; i < boot_trace_size_bound; ++i) {
        ...
        boot_line_bits.insert(
            ...
            boot_lines[boot_trace_size_bound - 1 - i]
                .contents_after->bits.begin(),
            boot_lines[boot_trace_size_bound - 1 - i]
                .contents_after->bits.end());

        pb_variable_array<FieldT> packed_boot_line =
            pb_variable_array<FieldT>(input_it, input_it + packed_line_size);
        std::advance(input_it, packed_line_size);

        unpack_boot_lines.emplace_back(multipacking_gadget<FieldT>(
            pb,
            boot_line_bits,
            packed_boot_line,
            max_chunk_size,
            FMT(annotation_prefix, " unpack_boot_lines_%zu", i)));
    }
    ...
    for (size_t i = 0; i < num_memory_lines; ++i) {
        routed_memory_lines.emplace_back(memory_line_variable_gadget<ramT>(
            pb,
            timestamp_size,
            pb.ap,
            FMT(annotation_prefix, " routed_memory_lines_%zu", i)));
    }
    ...
    routing_network.reset(new as_waksman_routing_gadget<FieldT>(
        pb,
        num_memory_lines,
        routing_inputs,
        routing_outputs,
        FMT(this->annotation_prefix, " routing_network")));
    ...
    for (size_t i = 0; i < time_bound; ++i) {
        execution_checkers.emplace_back(ram_cpu_checker<ramT>(
            pb,
...
void ram_universal_gadget<ramT>::generate_r1cs_constraints()
{
    ...
    for (size_t i = 0; i < this->pb.ap.cpu_state_size(); ++i) {
        generate_r1cs_equals_const_constraint<FieldT>(
            this->pb, execution_lines[0].cpu_state[i], FieldT::zero());
    }

    /* ensure increasing timestamps */
    for (size_t i = 0; i < num_memory_lines; ++i) {
        generate_r1cs_equals_const_constraint<FieldT>(
            this->pb, unrouted_memory_lines[i]->timestamp->packed, FieldT(i));
    }
    ...
    for (size_t i = 0; i < time_bound; ++i) {
        this->pb.add_r1cs_constraint(
            r1cs_constraint<FieldT>(
                1,
                load_instruction_lines[i].contents_before->packed,
    ...
    generate_r1cs_equals_const_constraint<FieldT>(
        this->pb,
        load_instruction_lines[0].address->packed,
        FieldT(this->pb.ap.initial_pc_addr()));

    /* ensure that the last state was an accepting one */
    generate_r1cs_equals_const_constraint<FieldT>(
        this->pb,
        execution_lines[time_bound].has_accepted,
        FieldT::one(),
    ...
    if (!libff::inhibit_profiling_info) {
        ...
        libff::print_indent();
        printf(
            "* Number of constraints / cycle: %0.1f\n",
        ...
        libff::print_indent();
        printf(
            "* Number of variables / cycle: %0.1f\n",
...
template<typename ramT>
void ram_universal_gadget<ramT>::generate_r1cs_witness(
    ...
    const ram_input_tape<ramT> &auxiliary_input)
{
    ...
    ra_memory mem_backend(
        1ul << (this->pb.ap.address_size()),
        this->pb.ap.value_size(),
        memory_after_boot);
    typename ram_input_tape<ramT>::const_iterator auxiliary_input_it =
        auxiliary_input.begin();

    this->pb.val(load_instruction_lines[0].address->packed) =
        FieldT(this->pb.ap.initial_pc_addr(), true);
    ...
    for (size_t i = 0; i < time_bound; ++i) {
        ...
        /* fill it in */
        const size_t load_store_addr =
            this->pb.val(execution_lines[i + 1].address->packed).as_ulong();
        const size_t load_store_prev_val =
            mem_backend.get_value(load_store_addr);

        this->pb.val(execution_lines[i + 1].contents_before->packed) =
            FieldT(load_store_prev_val, true);
        execution_lines[i + 1]
            .contents_before->generate_r1cs_witness_from_packed();

        /* then execute the rest of the instruction */
        execution_checkers[i].generate_r1cs_witness_other(
            auxiliary_input_it, auxiliary_input.end());
        ...
        const size_t load_store_next_val =
            this->pb.val(execution_lines[i + 1].contents_after->packed)
                .as_ulong();
        mem_backend.set_value(load_store_addr, load_store_next_val);

        /* the next PC address was passed in a bit form, so maintain packed form
         * as well */
    ...
    */
    ...
    for (size_t i = 0; i < this->num_memory_lines; ++i) {
        ...
        const auto it = std::upper_bound(
            mem_pairs.begin(),
            mem_pairs.end(),
            std::make_pair(address, timestamp));
...
void ram_universal_gadget<ramT>::print_execution_trace() const
{
    for (size_t i = 0; i < boot_trace_size_bound; ++i) {
        printf(
            "Boot process at t=#%zu: store %zu at %zu\n",
            i,
    ...
    for (size_t i = 0; i < time_bound; ++i) {
        ...
        printf(
            "  Loaded instruction %zu from address %zu (ts = %zu)\n",
            this->pb.val(load_instruction_lines[i].contents_after->packed)
                .as_ulong(),
            this->pb.val(load_instruction_lines[i].address->packed).as_ulong(),
            this->pb.val(load_instruction_lines[i].timestamp->packed)
                .as_ulong());
        ...
        printf(
            "  Memory operation executed: addr = %zu, contents_before = %zu, "
            "contents_after = %zu (ts_before = %zu, ts_after = %zu)\n",
            this->pb.val(execution_lines[i + 1].address->packed).as_ulong(),
            this->pb.val(execution_lines[i + 1].contents_before->packed)
                .as_ulong(),
            this->pb.val(execution_lines[i + 1].contents_after->packed)
...
void ram_universal_gadget<ramT>::print_memory_trace() const
{
    for (size_t i = 0; i < num_memory_lines; ++i) {
        ...
        printf(
            "  Time side  : ts = %zu, address = %zu, contents_before = %zu, "
            "contents_after = %zu\n",
            this->pb.val(unrouted_memory_lines[i]->timestamp->packed)
                .as_ulong(),
            this->pb.val(unrouted_memory_lines[i]->address->packed).as_ulong(),
            this->pb.val(unrouted_memory_lines[i]->contents_before->packed)
                .as_ulong(),
            this->pb.val(unrouted_memory_lines[i]->contents_after->packed)
                .as_ulong());
        printf(
            "  Memory side: ts = %zu, address = %zu, contents_before = %zu, "
            "contents_after = %zu\n",
            this->pb.val(routed_memory_lines[i].timestamp->packed).as_ulong(),
            this->pb.val(routed_memory_lines[i].address->packed).as_ulong(),
            this->pb.val(routed_memory_lines[i].contents_before->packed)
                .as_ulong(),
            this->pb.val(routed_memory_lines[i].contents_after->packed)
                .as_ulong());
    }
}

template<typename ramT>
size_t ram_universal_gadget<ramT>::packed_input_element_size(
...
template<typename ramT>
size_t ram_universal_gadget<ramT>::packed_input_size(
    const ram_architecture_params<ramT> &ap, const size_t boot_trace_size_bound)
{
