 ...
 *****************************************************************************
 ...
 - "ppzkSNARK" = "Pre-Processing Zero-Knowledge Succinct Non-interactive
...
template<typename ppT> class uscs_ppzksnark_proving_key;
...
public:
    ...
    uscs_ppzksnark_proving_key<ppT> &operator=(
        const uscs_ppzksnark_proving_key<ppT> &other) = default;
    uscs_ppzksnark_proving_key(const uscs_ppzksnark_proving_key<ppT> &other) =
        default;
    uscs_ppzksnark_proving_key(uscs_ppzksnark_proving_key<ppT> &&other) =
        default;
    uscs_ppzksnark_proving_key(
        libff::G1_vector<ppT> &&V_g1_query,
        libff::G1_vector<ppT> &&alpha_V_g1_query,
    ...
    size_t G2_size() const { return V_g2_query.size(); }

    size_t G1_sparse_size() const { return G1_size(); }

    size_t G2_sparse_size() const { return G2_size(); }

    size_t size_in_bits() const
    {
        return libff::G1<ppT>::size_in_bits() * G1_size() +
               libff::G2<ppT>::size_in_bits() * G2_size();
    }
    ...
    friend std::istream &operator>>
        <ppT>(std::istream &in, uscs_ppzksnark_proving_key<ppT> &pk);
};

/******************************* Verification key ****************************/

template<typename ppT> class uscs_ppzksnark_verification_key;
...
public:
    ...
    size_t G1_size() const { return encoded_IC_query.size(); }

    size_t G2_size() const { return 3; }

    size_t size_in_bits() const
    {
        return encoded_IC_query.size_in_bits() +
               3 * libff::G2<ppT>::size_in_bits();
    }
    ...
    friend std::ostream &operator<<<ppT>(
        std::ostream &out, const uscs_ppzksnark_verification_key<ppT> &vk);
    friend std::istream &operator>>
        <ppT>(std::istream &in, uscs_ppzksnark_verification_key<ppT> &vk);

    static uscs_ppzksnark_verification_key<ppT> dummy_verification_key(
        const size_t input_size);
};

/************************ Processed verification key *************************/

template<typename ppT> class uscs_ppzksnark_processed_verification_key;
...
public:
    ...
    bool operator==(
        const uscs_ppzksnark_processed_verification_key &other) const;
    friend std::ostream &operator<<<ppT>(
        std::ostream &out,
        const uscs_ppzksnark_processed_verification_key<ppT> &pvk);
...
/**
 * A key pair for the USCS ppzkSNARK, which consists of a proving key and a
 * verification key.
 */
template<typename ppT> class uscs_ppzksnark_keypair
{
public:
...
template<typename ppT> class uscs_ppzksnark_proof;
...
public:
    ...
    {
        return (
            V_g1.is_well_formed() && alpha_V_g1.is_well_formed() &&
            H_g1.is_well_formed() && V_g2.is_well_formed());
    }
    ...
    friend std::ostream &operator<<<ppT>(
        std::ostream &out, const uscs_ppzksnark_proof<ppT> &proof);
    friend std::istream &operator>>
        <ppT>(std::istream &in, uscs_ppzksnark_proof<ppT> &proof);
};
 ...
 *
 * Given a USCS constraint system CS, this algorithm produces proving and
 ...
 * Above, CS is the USCS constraint system that was given as input to the
 * generator algorithm.
 */
template<
    typename ppT,
