        ...
        while (1) {
            ...
            if (route_left) {
                ...
                if (permutation.get(to_route) == hi) {
                    ...
                    /**
                     * We know that the corresponding switch on the right-hand
                     * side cannot be set, so we set it according to the
                     * incoming wire.
                     */
                    assert(
                ...
                /**
                 * We have arrived on the right-hand side, so the switch setting
                 * is fixed. Next, we back route from here.
                 */
                const size_t rhs_switch =
                    as_waksman_get_canonical_row_idx(lo, to_route);
